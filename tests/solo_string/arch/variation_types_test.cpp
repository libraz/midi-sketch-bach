...
TEST(VariationTypesTest, StandardOrderIsValid) {
  ...
  std::vector<VariationRole> roles = {VariationRole::Establish,   VariationRole::Develop,
                                      ...
                                      VariationRole::Illuminate,  VariationRole::Destabilize,
                                      VariationRole::Accumulate,  VariationRole::Accumulate,
...
TEST(VariationTypesTest, MinimalValidOrder) {
  ...
  std::vector<VariationRole> roles = {
      VariationRole::Establish,  VariationRole::Develop,     VariationRole::Destabilize,
      VariationRole::Illuminate, VariationRole::Destabilize, VariationRole::Accumulate,
      VariationRole::Accumulate, VariationRole::Accumulate,  VariationRole::Resolve};
  EXPECT_TRUE(isRoleOrderValid(roles));
}
...
TEST(VariationTypesTest, ReversedOrderIsInvalid) {
  std::vector<VariationRole> roles = {VariationRole::Resolve,    VariationRole::Accumulate,
                                      VariationRole::Accumulate, VariationRole::Accumulate,
                                      VariationRole::Illuminate, VariationRole::Destabilize,
                                      VariationRole::Develop,    VariationRole::Establish};
...
TEST(VariationTypesTest, TwoAccumulateIsInvalid) {
  std::vector<VariationRole> roles = {VariationRole::Establish,   VariationRole::Develop,
                                      VariationRole::Destabilize, VariationRole::Illuminate,
                                      VariationRole::Destabilize, VariationRole::Accumulate,
                                      VariationRole::Accumulate,  VariationRole::Resolve};
  EXPECT_FALSE(isRoleOrderValid(roles));
}
...
TEST(VariationTypesTest, ZeroAccumulateIsInvalid) {
  std::vector<VariationRole> roles = {VariationRole::Establish,   VariationRole::Develop,
                                      VariationRole::Destabilize, VariationRole::Illuminate,
                                      VariationRole::Destabilize, VariationRole::Resolve};
  EXPECT_FALSE(isRoleOrderValid(roles));
}

TEST(VariationTypesTest, ResolveNotAtEndIsInvalid) {
  std::vector<VariationRole> roles = {VariationRole::Establish,
                                      VariationRole::Resolve,  // Resolve in wrong position
                                      VariationRole::Develop,    VariationRole::Destabilize,
                                      VariationRole::Illuminate, VariationRole::Destabilize,
                                      VariationRole::Accumulate, VariationRole::Accumulate,
...
TEST(VariationTypesTest, DevelopBeforeEstablishIsInvalid) {
  std::vector<VariationRole> roles = {VariationRole::Develop,  // Should come after Establish
                                      VariationRole::Establish,  VariationRole::Destabilize,
                                      VariationRole::Illuminate, VariationRole::Destabilize,
                                      VariationRole::Accumulate, VariationRole::Accumulate,
                                      VariationRole::Accumulate, VariationRole::Resolve};
  EXPECT_FALSE(isRoleOrderValid(roles));
