...
TEST(BassRealizerTest, SimpleProducesOneNotePerEntry) {
  ...
  auto notes = realizeBass(scheme, kDMinor, VariationRole::Establish, kViolinLow, kViolinHigh, 42);
...
TEST(BassRealizerTest, ResolveAlsoProducesOneNotePerEntry) {
  auto scheme = ChaconneScheme::createStandardDMinor();
  auto notes = realizeBass(scheme, kDMinor, VariationRole::Resolve, kViolinLow, kViolinHigh, 42);
  EXPECT_EQ(notes.size(), 7u);
}
...
TEST(BassRealizerTest, WalkingStrongBeatMatchesSimpleRoot) {
  ...
  auto simple_notes =
      realizeBass(scheme, kDMinor, VariationRole::Establish, kViolinLow, kViolinHigh, 42);
  auto walking_notes =
      realizeBass(scheme, kDMinor, VariationRole::Develop, kViolinLow, kViolinHigh, 42);
    ...
    for (const auto& note : walking_notes) {
      if (note.start_tick == entry_start) {
        ...
        EXPECT_EQ(note.pitch, simple_notes[eidx].pitch)
            << "Entry " << eidx << " walking strong beat pitch " << static_cast<int>(note.pitch)
            << " differs from simple root pitch " << static_cast<int>(simple_notes[eidx].pitch);
        break;
      }
    }
    EXPECT_TRUE(found) << "No walking note found at entry " << eidx << " start tick "
                       << entry_start;
  }
}
...
TEST(BassRealizerTest, SyncopatedDensityWithinLimit) {
  auto scheme = ChaconneScheme::createStandardDMinor();
  auto notes =
      realizeBass(scheme, kDMinor, VariationRole::Destabilize, kViolinLow, kViolinHigh, 42);
  ...
  if (total_notes > 0) {
    ...
    EXPECT_LE(synco_ratio, 0.40f) << "Syncopation density " << synco_ratio << " exceeds 40% limit";
  }
}
...
TEST(BassRealizerTest, ElaborateProducesMoreNotesThanSimple) {
  ...
  auto simple_notes =
      realizeBass(scheme, kDMinor, VariationRole::Establish, kViolinLow, kViolinHigh, 42);
  auto elaborate_notes =
      realizeBass(scheme, kDMinor, VariationRole::Accumulate, kViolinLow, kViolinHigh, 42);
...
TEST(BassRealizerTest, AllNotesWithinRegisterBounds) {
  ...
  // Test all roles.
  std::vector<VariationRole> roles = {VariationRole::Establish,   VariationRole::Develop,
                                      VariationRole::Destabilize, VariationRole::Illuminate,
                                      VariationRole::Accumulate,  VariationRole::Resolve};

  for (auto role : roles) {
    auto notes = realizeBass(scheme, kDMinor, role, kViolinLow, kViolinHigh, 42);

    BassRegisterProfile profile = getBassRegisterProfile(role, kViolinLow, kViolinHigh);

    for (const auto& note : notes) {
      EXPECT_GE(note.pitch, profile.effective_low)
          << "Note pitch " << static_cast<int>(note.pitch) << " below register low "
          << static_cast<int>(profile.effective_low) << " for role " << variationRoleToString(role)
      ...
      EXPECT_LE(note.pitch, profile.effective_high)
          ...
          << static_cast<int>(profile.effective_high) << " for role " << variationRoleToString(role)
          << " at tick " << note.start_tick;
    }
  }
}
  ...
  for (auto role : roles) {
    ...
    for (const auto& note : notes) {
      EXPECT_EQ(note.source, BachNoteSource::ChaconneBass)
          << "Note at tick " << note.start_tick << " has wrong source for role "
...
TEST(BassRealizerTest, SameSeedProducesSameResult) {
  ...
  auto notes_a = realizeBass(scheme, kDMinor, VariationRole::Develop, kViolinLow, kViolinHigh, 42);
  auto notes_b = realizeBass(scheme, kDMinor, VariationRole::Develop, kViolinLow, kViolinHigh, 42);

  ASSERT_EQ(notes_a.size(), notes_b.size());
  for (size_t i = 0; i < notes_a.size(); ++i) {
    EXPECT_EQ(notes_a[i].pitch, notes_b[i].pitch) << "Pitch mismatch at note " << i;
    EXPECT_EQ(notes_a[i].start_tick, notes_b[i].start_tick) << "Start tick mismatch at note " << i;
    EXPECT_EQ(notes_a[i].duration, notes_b[i].duration) << "Duration mismatch at note " << i;
  }
}
...
TEST(BassRealizerTest, DifferentSeedsProduceDifferentResults) {
  ...
  std::vector<VariationRole> rng_roles = {VariationRole::Develop, VariationRole::Destabilize,
                                          VariationRole::Accumulate};

  for (auto role : rng_roles) {
    auto notes_a = realizeBass(scheme, kDMinor, role, kViolinLow, kViolinHigh, 42);
    auto notes_b = realizeBass(scheme, kDMinor, role, kViolinLow, kViolinHigh, 99);
    ...
    EXPECT_TRUE(any_diff) << "Different seeds produced identical output for role "
                          << variationRoleToString(role);
  }
}
...
TEST(BassRealizerTest, AccumulateRegisterExpandsWithIndex) {
  auto profile_0 = getBassRegisterProfile(VariationRole::Accumulate, kViolinLow, kViolinHigh, 0);
  auto profile_1 = getBassRegisterProfile(VariationRole::Accumulate, kViolinLow, kViolinHigh, 1);
  auto profile_2 = getBassRegisterProfile(VariationRole::Accumulate, kViolinLow, kViolinHigh, 2);
...
TEST(BassRealizerTest, EmptySchemeReturnsNoNotes) {
  ...
  auto notes =
      realizeBass(empty_scheme, kDMinor, VariationRole::Establish, kViolinLow, kViolinHigh, 42);
...
TEST(BassRealizerTest, AllNotesHaveNonZeroDuration) {
  ...
  for (auto role : roles) {
    ...
    for (const auto& note : notes) {
      EXPECT_GT(note.duration, 0u) << "Note with zero duration at tick " << note.start_tick
...
TEST(BassRealizerTest, WalkingProducesMoreNotesThanSimple) {
  auto scheme = ChaconneScheme::createStandardDMinor();
  auto simple = realizeBass(scheme, kDMinor, VariationRole::Establish, kViolinLow, kViolinHigh, 42);
  auto walking = realizeBass(scheme, kDMinor, VariationRole::Develop, kViolinLow, kViolinHigh, 42);

  // Walking bass fills weak beats, so it should have more notes.
  EXPECT_GT(walking.size(), simple.size())
      << "Walking (" << walking.size() << ") should have more notes than Simple (" << simple.size()
      << ")";
}
...
TEST(BassRealizerTest, LyricalProducesNotes) {
  auto scheme = ChaconneScheme::createStandardDMinor();
  auto notes = realizeBass(scheme, kDMinor, VariationRole::Illuminate, kViolinLow, kViolinHigh, 42);
  ...
  for (auto role : roles) {
    ...
    for (size_t i = 1; i < notes.size(); ++i) {
      EXPECT_GE(notes[i].start_tick, notes[i - 1].start_tick)
          << "Notes not sorted at index " << i << " for role " << variationRoleToString(role);
    }
  }
}
