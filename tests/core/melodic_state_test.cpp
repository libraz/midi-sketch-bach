...
// ===========================================================================
// CarryMelodicStateTest
// ===========================================================================

class CarryMelodicStateTest : public ::testing::Test {
 protected:
  MelodicState makePrevState() {
    MelodicState prev{};
    prev.last_direction = 1;
    prev.direction_run_length = 5;
    prev.last_skip_size = 4;
    prev.last_large_leap = 7;
    prev.phrase_progress = 0.8f;
    prev.prev_was_reversal = true;
    prev.consecutive_leap_count = 3;
    prev.contour = {PhraseContour::Wave, 0.5f, 0.30f};
    return prev;
  }
};

TEST_F(CarryMelodicStateTest, CadenceBoundaryResetsRunLength) {
  MelodicState prev = makePrevState();
  MelodicState next = carryMelodicState(prev, BoundaryType::Cadence);
  EXPECT_EQ(next.direction_run_length, 0);
}

TEST_F(CarryMelodicStateTest, CadenceBoundaryNeutralContour) {
  MelodicState prev = makePrevState();
  MelodicState next = carryMelodicState(prev, BoundaryType::Cadence);
  EXPECT_EQ(next.contour.shape, PhraseContour::Neutral);
}

TEST_F(CarryMelodicStateTest, DevelopmentPreservesContour) {
  MelodicState prev = makePrevState();
  MelodicState next = carryMelodicState(prev, BoundaryType::Development);
  EXPECT_EQ(next.contour.shape, prev.contour.shape);
  EXPECT_FLOAT_EQ(next.contour.peak_progress, prev.contour.peak_progress);
  EXPECT_FLOAT_EQ(next.contour.strength, prev.contour.strength);
}

TEST_F(CarryMelodicStateTest, ReentryFreshArch) {
  MelodicState prev = makePrevState();
  MelodicState next = carryMelodicState(prev, BoundaryType::Reentry);
  EXPECT_EQ(next.contour.shape, PhraseContour::Arch);
  EXPECT_FLOAT_EQ(next.contour.peak_progress, 0.4f);
  EXPECT_FLOAT_EQ(next.contour.strength, 0.25f);
}

TEST_F(CarryMelodicStateTest, LeapTrackingAlwaysReset) {
  MelodicState prev = makePrevState();
  ASSERT_NE(prev.last_skip_size, 0);
  ASSERT_NE(prev.last_large_leap, 0);
  ASSERT_NE(prev.consecutive_leap_count, 0);

  for (auto boundary : {BoundaryType::Cadence, BoundaryType::Development,
                         BoundaryType::Reentry}) {
    MelodicState next = carryMelodicState(prev, boundary);
    EXPECT_EQ(next.last_skip_size, 0)
        << "last_skip_size not reset for boundary "
        << static_cast<int>(boundary);
    EXPECT_EQ(next.last_large_leap, 0)
        << "last_large_leap not reset for boundary "
        << static_cast<int>(boundary);
    EXPECT_EQ(next.consecutive_leap_count, 0)
        << "consecutive_leap_count not reset for boundary "
        << static_cast<int>(boundary);
  }
}

TEST_F(CarryMelodicStateTest, PhraseProgressAlwaysReset) {
  MelodicState prev = makePrevState();
  ASSERT_GT(prev.phrase_progress, 0.0f);

  for (auto boundary : {BoundaryType::Cadence, BoundaryType::Development,
                         BoundaryType::Reentry}) {
    MelodicState next = carryMelodicState(prev, boundary);
    EXPECT_FLOAT_EQ(next.phrase_progress, 0.0f)
        << "phrase_progress not reset for boundary "
        << static_cast<int>(boundary);
  }
}

TEST_F(CarryMelodicStateTest, DirectionPreservedForNonCadence) {
  MelodicState prev = makePrevState();
  ASSERT_EQ(prev.last_direction, 1);

  // Development and Reentry preserve direction.
  MelodicState dev = carryMelodicState(prev, BoundaryType::Development);
  EXPECT_EQ(dev.last_direction, 1);

  MelodicState re = carryMelodicState(prev, BoundaryType::Reentry);
  EXPECT_EQ(re.last_direction, 1);
}

TEST_F(CarryMelodicStateTest, RunLengthCappedAtTwo) {
  MelodicState prev = makePrevState();
  ASSERT_EQ(prev.direction_run_length, 5);

  // Development and Reentry cap run_length at 2.
  MelodicState dev = carryMelodicState(prev, BoundaryType::Development);
  EXPECT_EQ(dev.direction_run_length, 2);

  MelodicState re = carryMelodicState(prev, BoundaryType::Reentry);
  EXPECT_EQ(re.direction_run_length, 2);
}

TEST_F(CarryMelodicStateTest, PrevWasReversalAlwaysReset) {
  MelodicState prev = makePrevState();
  ASSERT_TRUE(prev.prev_was_reversal);

  for (auto boundary : {BoundaryType::Cadence, BoundaryType::Development,
                         BoundaryType::Reentry}) {
    MelodicState next = carryMelodicState(prev, boundary);
    EXPECT_FALSE(next.prev_was_reversal)
        << "prev_was_reversal not reset for boundary "
        << static_cast<int>(boundary);
  }
}
