...
TEST(ExpositionTest, BuildExposition_CountersubjectPitchesMatchSource) {
  Subject subject = makeTestSubject();
  Answer answer = makeTestAnswer(subject);
  Countersubject counter = makeTestCountersubject(subject);
  ...
  for (size_t idx = 0; idx < counter.notes.size(); ++idx) {
    ...
    EXPECT_EQ(voice0_notes[note_idx].pitch, counter.notes[idx].pitch)
        << "Countersubject note " << idx << " pitch mismatch in voice 0";
