...
| `texture-gate` | Sweep seed × form and check fugue texture gates (active voices, silence, repeated runs, compass, entry-plan nonperiodicity, model score). | `bach_cli`, bach-mcp | JSON to stdout (`--out` optional) | humans / CI gating |
...
| `mirror.py` | **Byte-stable mirror constants** of C++ `harness_fixture.cpp` (PHASE15–25 layouts, subjects, required-bit sets). Do not reformat or edit values — the 12 `test_*_mirror.py` drift guards depend on them. |
