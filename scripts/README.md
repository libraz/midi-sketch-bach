...
| `qualify-subjects` | Qualify pool candidates through the real product path: each candidate is patched into all subject slots of a throwaway git worktree, `bach_cli` is rebuilt incrementally, and the fugue-family forms are judged with the full texture-gate axes plus cadence/leading-tone provenance bits. Resumable JSONL; `--baseline` judges the shipped subjects; `--catalog-out` renders `subject_catalog.inc`. | `backup/subject_pool.json`, git worktree, bach-mcp | `backup/subject_qualify.jsonl`, optional `.inc` | subject catalog (runtime switch) |
