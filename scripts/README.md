...
| `sweep-score` | Sweep form × character × key × seed and report the structure axes: out-of-key share (counted against the internal C tonic, split by mode), exact-duplicate bar share, bass stepwise share, velocity level count, note-gap share, plus bach-mcp's length-invariant model probability, its six KL components' excess over length-matched Bach, and the counterpoint corpus excess distance. Per-form rows are medians over the accepted configurations; refused configurations are listed rather than dropped. `--baseline` prints the delta against an earlier report. | `bach_cli`, bach-mcp | text report to stdout (`--out` JSON optional) | humans / before-after comparison |
...
| `reachability` | Drift guard for declarations product code has stopped reaching: every `CharacterProfile` field must be read through a member access outside its declaring pair, and every `SubjectCharacter` / `FormType` value must reach a distinct outcome in the form director (its own case body, or an explicit rejection). Declarations are located by symbol over the `src/` tree, so a file split does not make an existing declaration look absent; an unlocatable subject exits 2 instead of passing on an empty set. | `src/` tree | text/JSON to stdout (`--json`, `--out`) | humans / drift guard |
...
| `sweep_score.py` | Sweep structure metrics (out-of-key, duplicate bars, bass stepwise motion, velocity levels, note gaps) joined with bach-mcp's corpus-calibrated axes; aggregates per form and diffs against a stored baseline |
...
| `reachability.py` | declared-but-unreached guards for `CharacterProfile` fields and `SubjectCharacter` / `FormType` values; locates its subjects by symbol over `src/` and raises rather than passing vacuously |
