...
def compute_alternation_shares(
    ...
    """Share of each line's notes lying inside a two-pitch shake, by line.

    A note belongs to a shake when it restates the pitch two notes back without
    restating the one immediately before it; a maximal run of four or more such
    notes is the figure the ceiling is about. Lines are keyed
    ``"v<voice>/<intent>"`` so a subject and the accompaniment written under it
...
def evaluate_generated_json(form: str, seed: int, generated_json: Path) -> GateCase:
    ...
    alternation_shares = compute_alternation_shares(notes, provenance)
    return GateCase(
        form=form,
        seed=seed,
        generated=True,
