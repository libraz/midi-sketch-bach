"""Solo String Arch (BWV1004 Chaconne) Phase16 C++/Python drift guard.

The load-bearing guard: the run_phase_closure.py PHASE16 mirror constants and
the phase16_scale_up walk must match the C++ Phase16 fixture source
(buildPhase16Fixture in src/composer/harness_fixture.cpp). A renamed constant or
...
def _parse_phase16_scale_pcs() -> tuple[int, ...]:
    """Recover the C-minor scale pitch classes from phase16InScale().
...
class Phase16CppDriftGuardTest(unittest.TestCase):
    """The Python PHASE16 mirror constants must equal the C++ source."""
    ...
    def test_ground_matches_cpp(self) -> None:
        ...
        self.assertEqual(
            cpp,
            rpc.PHASE16_GROUND,
            "PHASE16_GROUND drifted from kGroundPitch in harness_fixture.cpp",
        )
    ...
    def test_var_t0_matches_cpp(self) -> None:
        ...
        self.assertEqual(
            cpp,
            rpc.PHASE16_VAR_T0,
            "PHASE16_VAR_T0 drifted from kVarT0 in harness_fixture.cpp",
        )

    def test_cmin_scale_matches_cpp(self) -> None:
        cpp = _parse_phase16_scale_pcs()
        self.assertEqual(
            cpp,
            tuple(sorted(rpc.PHASE16_CMIN_SCALE)),
            "PHASE16_CMIN_SCALE drifted from phase16InScale in harness_fixture.cpp",
        )
    ...
    def test_block_notes_per_beat_shape(self) -> None:
        ...
        self.assertEqual(rpc.PHASE16_BLOCK_NOTES_PER_BEAT, (1, 2, 4, 4))


class Phase16ScaleWalkTest(unittest.TestCase):
    """phase16_scale_up must walk C-natural-minor degrees, matching C++."""
    ...
    def test_half_step_from_bb3(self) -> None:
        ...
        self.assertEqual(rpc.phase16_scale_up(58, 1), 60)

    def test_zero_steps_is_identity(self) -> None:
        self.assertEqual(rpc.phase16_scale_up(55, 0), 55)


class Phase16RequiredBitsTest(unittest.TestCase):
    """The Phase16 required-bit set must track provenance.h RuleBit values."""

    def test_required_bits_are_49_50_51(self) -> None:
        self.assertEqual(rpc.PHASE16_REQUIRED_BITS, (49, 50, 51))
