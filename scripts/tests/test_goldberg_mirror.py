...
def _goldbergVariations_body() -> str:
    """Return the source body following the buildGoldbergVariationsFixture declaration.

    Earlier builders (Chaconne..24) also declare kGroundPitch / kBarRoot /
    kBarMinor style arrays; anchoring on the GoldbergVariations builder body ensures only
    the GoldbergVariations arrays / block table are captured.
    """
    src = FIXTURE_CPP.read_text(encoding="utf-8")
    body = src.split("buildGoldbergVariationsFixture", 1)
    if len(body) < 2:
        raise AssertionError("could not locate buildGoldbergVariationsFixture in harness_fixture.cpp")
...
def _parse_int_array(name: str) -> tuple[int, ...]:
    ...
    body = _goldbergVariations_body()
    match = re.search(rf"{name}\s*\[\s*kCycleBars\s*\]\s*=\s*\{{([^{{}}]*?)\}}", body, re.S)
    if match is None:
        raise AssertionError(f"could not locate {name}[kCycleBars] in buildGoldbergVariationsFixture")
...
def _parse_block_spec() -> tuple[tuple[int, int, int, bool], ...]:
    ...
    if match is None:
        raise AssertionError("could not locate kBlockSpec[kBlocks] in buildGoldbergVariationsFixture")
...
class GoldbergVariationsCppDriftGuardTest(unittest.TestCase):
    """The Python GOLDBERG_VARIATIONS mirror constants must equal the C++ source."""

    def test_ground_matches_cpp(self) -> None:
        self.assertEqual(
            _parse_int_array("kGroundPitch"),
            rpc.GOLDBERG_VARIATIONS_GROUND,
            "GOLDBERG_VARIATIONS_GROUND drifted from kGroundPitch[kCycleBars] in buildGoldbergVariationsFixture",
        )

    def test_bar_root_matches_cpp(self) -> None:
        self.assertEqual(
            _parse_int_array("kBarRoot"),
            rpc.GOLDBERG_VARIATIONS_BAR_ROOT,
            "GOLDBERG_VARIATIONS_BAR_ROOT drifted from kBarRoot[kCycleBars] in buildGoldbergVariationsFixture",
        )

    def test_bar_minor_matches_cpp(self) -> None:
        self.assertEqual(
            _parse_bool_array("kBarMinor"),
            rpc.GOLDBERG_VARIATIONS_BAR_MINOR,
            "GOLDBERG_VARIATIONS_BAR_MINOR drifted from kBarMinor[kCycleBars] in buildGoldbergVariationsFixture",
        )
    ...
    def test_block_spec_matches_cpp(self) -> None:
        ...
        self.assertEqual(
            cpp,
            rpc.GOLDBERG_VARIATIONS_BLOCK_SPEC,
            "GOLDBERG_VARIATIONS_BLOCK_SPEC drifted from kBlockSpec[kBlocks] in buildGoldbergVariationsFixture",
        )
    ...
    def test_ground_period_matches_cpp(self) -> None:
        ...
        self.assertEqual(rpc.GOLDBERG_VARIATIONS_GROUND_PERIOD, 7680)
    ...
    def test_constants_are_4_bar_cycle(self) -> None:
        self.assertEqual(len(rpc.GOLDBERG_VARIATIONS_GROUND), 4)
        self.assertEqual(len(rpc.GOLDBERG_VARIATIONS_BAR_ROOT), 4)
        self.assertEqual(len(rpc.GOLDBERG_VARIATIONS_BAR_MINOR), 4)
        self.assertEqual(len(rpc.GOLDBERG_VARIATIONS_BLOCK_SPEC), 5)
...
class GoldbergVariationsSequenceShapeTest(unittest.TestCase):
...
class GoldbergVariationsScaleWalkTest(unittest.TestCase):
    """The GoldbergVariations variation figuration reuses the OrganPrelude C-major scale walk."""
    ...
    def test_aria_chord_anchor_is_scale_tone(self) -> None:
        ...
        for root_pc, expect in zip((0, 5, 7, 9), (72, 77, 79, 81)):
            ...
            self.assertEqual(rpc.organPrelude_scale_up(chord_start, 0), expect)
