"""Keyboard suite KeyboardSuite C++/Python drift guard.

The load-bearing guard: the bachlib.mirror KEYBOARD_SUITE mirror constants
(KEYBOARD_SUITE_BAR_ROOT, KEYBOARD_SUITE_BAR_MINOR, KEYBOARD_SUITE_GROUND, KEYBOARD_SUITE_GROUND_PERIOD, and
the kMovements[5] window / carrier / kind / base_midi table) must match the C++
KeyboardSuite fixture source (the kBarRoot[4] / kBarMinor[4] / kGroundPitch[4]
initialisers and the kMovements[5] movement table in buildKeyboardSuiteFixture,
...
diverge from the CLI output; this drift guard fails instead. The KeyboardSuite fixture
...
values. The suite figuration reuses the OrganPrelude C-major scale walk, so the
scalar-wave sanity asserts exercise organPrelude_scale_up directly.
"""
...
def _keyboardSuite_body() -> str:
    """Return the source body following the buildKeyboardSuiteFixture declaration.

    Earlier builders (OrganPrelude..22) also declare kBarRoot / kBarMinor /
    kGroundPitch style arrays; anchoring on the KeyboardSuite builder body ensures only
    the KeyboardSuite arrays / kMovements are captured.
    """
    src = FIXTURE_CPP.read_text(encoding="utf-8")
    body = src.split("buildKeyboardSuiteFixture", 1)
    if len(body) < 2:
        raise AssertionError("could not locate buildKeyboardSuiteFixture in harness_fixture.cpp")
    return body[1]


def _parse_int_array(name: str) -> tuple[int, ...]:
    """Parse a `<name>[N] = { ... }` integer initialiser from the KeyboardSuite body."""
    body = _keyboardSuite_body()
    match = re.search(rf"{name}\s*\[[^\]]*\]\s*=\s*\{{([^{{}}]*?)\}}", body, re.S)
    if match is None:
        raise AssertionError(f"could not locate {name}[...] in buildKeyboardSuiteFixture")
    return tuple(int(tok) for tok in re.split(r"[,\s]+", match.group(1)) if tok)


def _parse_bool_array(name: str) -> tuple[bool, ...]:
    """Parse a `<name>[N] = { ... }` bool initialiser from the KeyboardSuite body."""
    body = _keyboardSuite_body()
    match = re.search(rf"{name}\s*\[[^\]]*\]\s*=\s*\{{([^{{}}]*?)\}}", body, re.S)
    if match is None:
        raise AssertionError(f"could not locate {name}[...] in buildKeyboardSuiteFixture")
...
class KeyboardSuiteCppDriftGuardTest(unittest.TestCase):
    """The Python KEYBOARD_SUITE mirror constants must equal the C++ source."""

    def test_bar_root_matches_cpp(self) -> None:
        self.assertEqual(
            _parse_int_array("kBarRoot"),
            rpc.KEYBOARD_SUITE_BAR_ROOT,
            "KEYBOARD_SUITE_BAR_ROOT drifted from kBarRoot[4] in buildKeyboardSuiteFixture",
        )

    def test_bar_minor_matches_cpp(self) -> None:
        self.assertEqual(
            _parse_bool_array("kBarMinor"),
            rpc.KEYBOARD_SUITE_BAR_MINOR,
            "KEYBOARD_SUITE_BAR_MINOR drifted from kBarMinor[4] in buildKeyboardSuiteFixture",
        )

    def test_ground_pitch_matches_cpp(self) -> None:
        self.assertEqual(
            _parse_int_array("kGroundPitch"),
            rpc.KEYBOARD_SUITE_GROUND,
            "KEYBOARD_SUITE_GROUND drifted from kGroundPitch[4] in buildKeyboardSuiteFixture",
        )

    def test_ground_period_matches_cpp(self) -> None:
        # ground_bass_period = kCycleBars (4) * kTicksPerBar (1920) = 7680.
        self.assertEqual(rpc.KEYBOARD_SUITE_GROUND_PERIOD, 4 * 1920)
        body = _keyboardSuite_body()
        self.assertIn(
            "ground_bass_period = static_cast<Tick>(kCycleBars) * kTicksPerBar",
            body,
            "KEYBOARD_SUITE_GROUND_PERIOD construction drifted in buildKeyboardSuiteFixture",
        )
    ...
    def test_movements_match_cpp(self) -> None:
        ...
        self.assertIsNotNone(match, "could not locate kMovements[5] in buildKeyboardSuiteFixture")
        block = match.group(1)
        ...
        self.assertEqual(
            parsed,
            rpc.KEYBOARD_SUITE_MOVEMENTS,
            "KEYBOARD_SUITE_MOVEMENTS drifted from kMovements[5] in buildKeyboardSuiteFixture",
        )

    def test_constants_shape(self) -> None:
        self.assertEqual(len(rpc.KEYBOARD_SUITE_BAR_ROOT), 4)
        self.assertEqual(len(rpc.KEYBOARD_SUITE_BAR_MINOR), 4)
        self.assertEqual(len(rpc.KEYBOARD_SUITE_GROUND), 4)
        self.assertEqual(len(rpc.KEYBOARD_SUITE_MOVEMENTS), 5)
        # The five movements tile bars 0..19 contiguously.
        self.assertEqual(
            tuple((f, l) for f, l, _c, _k, _b in rpc.KEYBOARD_SUITE_MOVEMENTS),
            ((0, 3), (4, 7), (8, 11), (12, 15), (16, 19)),
        )
        # Carriers: Figuration / Fantasia / Fantasia / Figuration / Fantasia.
        self.assertEqual(
            tuple(c for _f, _l, c, _k, _b in rpc.KEYBOARD_SUITE_MOVEMENTS),
            (0, 1, 1, 0, 1),
        )
        # Kinds: sixteenths / eighths / halves / eighths / sixteenths.
        self.assertEqual(
            tuple(k for _f, _l, _c, k, _b in rpc.KEYBOARD_SUITE_MOVEMENTS),
            (1, 0, 2, 0, 1),
        )
        # Register bases C5 / C5 / C5 / E5 / E5.
        self.assertEqual(
            tuple(b for _f, _l, _c, _k, b in rpc.KEYBOARD_SUITE_MOVEMENTS),
            (72, 72, 72, 76, 76),
        )


class KeyboardSuiteRequiredBitsTest(unittest.TestCase):
    """The KeyboardSuite required-bit set must track provenance.h RuleBit values."""
    ...
    def test_required_bits_set(self) -> None:
        ...
        self.assertEqual(rpc.KEYBOARD_SUITE_REQUIRED_BITS, (52, 63, 49))
...
class KeyboardSuiteScaleWalkTest(unittest.TestCase):
    """The KeyboardSuite figuration reuses the OrganPrelude C-major scale walk."""
    ...
    def test_two_degrees_from_e5_is_g5(self) -> None:
        ...
        self.assertEqual(rpc.organPrelude_scale_up(76, 2), 79)


class KeyboardSuiteSequenceShapeTest(unittest.TestCase):
    ...
    def test_ground_register_below_dance(self) -> None:
        ...
        self.assertEqual(set(gr), set(rpc.KEYBOARD_SUITE_GROUND))
