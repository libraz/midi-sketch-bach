...
bool isLeadingTone(uint8_t pitch, Key key, ScaleType scale) {
  ...
  if (scale == ScaleType::NaturalMinor)
    return false;
...
bool isTendencyResolution(uint8_t prev_pitch, uint8_t curr_pitch, Key key, ScaleType scale) {
  ...
  if (prev_pc == fourth_degree_pc) {
    bool is_minor = (scale == ScaleType::NaturalMinor || scale == ScaleType::HarmonicMinor ||
                     scale == ScaleType::MelodicMinor || scale == ScaleType::Dorian);
  ...
  if (prev_pc == seventh_pc || prev_pc == minor_seventh_pc) {
    ...
    if (descent >= 1 && descent <= 2)
      return true;  // Descending step.
  }
...
bool isSequencePattern(const uint8_t* pitches, int count) {
  ...
  for (int idx = 0; idx < 4; ++idx) {
    intervals[idx] = static_cast<int>(pitches[idx + 1]) - static_cast<int>(pitches[idx]);
  }
...
int resolveLeaps(std::vector<NoteEvent>& notes, const LeapResolutionParams& params) {
  if (notes.size() < 3 || params.num_voices == 0)
    return 0;
  ...
  for (uint8_t vid = 0; vid < params.num_voices; ++vid) {
    ...
    for (int pos = 0; pos + 2 < len; ++pos) {
      ...
      if (abs_leap < params.leap_threshold)
        continue;

      if (params.applies_to_leap_source && !params.applies_to_leap_source(notes[i1].source)) {
        continue;
      }
      ...
      // lands on a subject note.
      {
        ...
        if (prot0 == ProtectionLevel::Immutable || prot1 == ProtectionLevel::Immutable) {
          continue;
        }
      }

      // P1: Source protection (Immutable -> skip).
      if (getProtectionLevel(notes[i2].source) != ProtectionLevel::Flexible)
        continue;
      ...
      if (notes[i1].start_tick / kTicksPerBar != notes[i2].start_tick / kTicksPerBar) {
        if (params.is_chord_tone) {
          ...
          if (params.is_chord_tone(notes[i2].start_tick, static_cast<uint8_t>(next_key))) {
            continue;
          }
      ...
      if (params.protect_strong_beat_resolution && notes[i2].start_tick % kTicksPerBeat == 0) {
        if (leap_detail::isLeadingTone(notes[i2].pitch, key, scale)) {
          if (abs_leap < 7)
            continue;  // P4 leap + strong-beat leading tone -> protect.
      ...
      if (contrary && step) {
        ...
        if (leap_detail::isTendencyResolution(notes[i1].pitch, notes[i2].pitch, key, scale)) {
      ...
      if (pos + 4 < len) {
        ...
        if (dir_a == dir_b && dir_b == dir_c && dir_a != 0) {
          int step_a =
              std::abs(static_cast<int>(notes[i2].pitch) - static_cast<int>(notes[i1].pitch));
          int step_b =
              std::abs(static_cast<int>(notes[i3].pitch) - static_cast<int>(notes[i2].pitch));
      ...
      if (params.protect_chord_tone_resolution && params.is_chord_tone &&
      ...
      if (pos + 4 < len) {
        uint8_t seq_pitches[5] = {notes[i0].pitch, notes[i1].pitch, notes[i2].pitch,
      ...
      for (int offset = 1; offset <= max_offset; ++offset) {
...

        {
          ...
          if (cand_u8 < low || cand_u8 > high)
            continue;
        }

        if (params.vertical_safe && !params.vertical_safe(notes[i2].start_tick, vid, cand_u8)) {
          continue;
        }

        bool cand_is_ct =
            params.is_chord_tone ? params.is_chord_tone(notes[i2].start_tick, cand_u8) : false;
        ...
        if (has_next && !anchor_exception) {
          ...
          if (std::abs(cand_to_next) >= params.leap_threshold) {
            ...
            if (!followup_resolves)
              continue;  // Reject: would cascade.
          }
        }
      ...
      if (best_pitch >= 0 && static_cast<uint8_t>(best_pitch) != notes[i2].pitch) {
        ...
        if (params.resolution_probability < 1.0f) {
          ...
          uint32_t tick_hash =
              notes[i2].start_tick * 2654435761u;  // NOLINT(readability-magic-numbers): Knuth's
