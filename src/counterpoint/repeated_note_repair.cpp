...
bool isProtectedSource(BachNoteSource source, const RepeatedNoteRepairParams& params) {
  bool structural = isStructuralSource(source);
  if (params.repair_coda && source == BachNoteSource::Coda) {
    structural = false;
  }
  return structural || source == BachNoteSource::GroundBass ||
...
bool tryCandidate(uint8_t base_pitch, int delta, Key key, ScaleType scale, uint8_t voice, Tick tick,
                  const RepeatedNoteRepairParams& params, uint8_t& out_pitch) {
...
int repairRepeatedNotes(std::vector<NoteEvent>& notes, const RepeatedNoteRepairParams& params) {
  if (notes.empty() || params.num_voices == 0)
    return 0;
  ...
  auto soundingPitchAt = [&](uint8_t v, Tick t) -> int {
    ...
    --it;
    size_t idx = *it;
    if (notes[idx].start_tick <= t && t < notes[idx].start_tick + notes[idx].duration) {
  ...
  for (uint8_t voice = 0; voice < params.num_voices; ++voice) {
    ...
    for (size_t pos = 1; pos <= idxs.size(); ++pos) {
      ...
      if (run_len > params.max_consecutive) {
        ...
        if (run_start > 0) {
          int diff =
              static_cast<int>(base_pitch) - static_cast<int>(notes[idxs[run_start - 1]].pitch);
        ...
        for (size_t jdx = run_start + static_cast<size_t>(params.max_consecutive);
          ...
          if (isProtectedSource(notes[note_idx].source, params))
            continue;
          if (notes[note_idx].source == BachNoteSource::Coda && params.repair_coda_voice >= 0 &&
              voice != static_cast<uint8_t>(params.repair_coda_voice)) {
            continue;
          }
          ...
          for (int dir : {step_dir, -step_dir}) {
            for (int delta = 1; delta <= 3; ++delta) {
              uint8_t cand = 0;
              if (tryCandidate(base_pitch, dir * delta, cur_key, cur_scale, voice,
                               notes[note_idx].start_tick, params, cand)) {
                int score = (4 - delta);  // proximity: 3, 2, 1
                if (dir == step_dir)
                  score += 3;  // preferred direction
          ...
          Tick beat = (notes[note_idx].start_tick % kTicksPerBar) / kTicksPerBeat;
          ...
          for (auto& cnd : candidates) {
            ...
            for (uint8_t ov = 0; ov < params.num_voices; ++ov) {
              ...
              if (other < 0)
                continue;
              int ivl =
                  interval_util::compoundToSimple(std::abs(static_cast<int>(cnd.pitch) - other));
              bool is_cons =
                  interval_util::isConsonance(ivl) || (params.num_voices >= 3 && ivl == 5);
              if (!is_cons) {
                consonant = false;
                break;
              }
          ...
          if (placed) {
            ...
            notes[note_idx].modified_by |= static_cast<uint8_t>(NoteModifiedBy::RepeatedNoteRep);
            ++total_modified;
          }
        }
      }
