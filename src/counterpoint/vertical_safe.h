...
inline bool checkVerticalConsonance(uint8_t cand_pitch, uint8_t voice, Tick tick,
  ...
  if (beat != 0 && beat != 2) {
    for (const auto& n : notes) {
      if (n.voice == voice)
        continue;
      if (n.start_tick + n.duration <= tick || n.start_tick > tick)
        continue;
      int simple = interval_util::compoundToSimple(absoluteInterval(cand_pitch, n.pitch));
  ...
  for (const auto& n : notes) {
    ...
    if (n.start_tick + n.duration <= tick || n.start_tick > tick)
      continue;
    int reduced = interval_util::compoundToSimple(absoluteInterval(cand_pitch, n.pitch));
...
inline std::function<bool(Tick, uint8_t, uint8_t)> makeVerticalSafeCallback(
    const HarmonicTimeline& timeline, const std::vector<NoteEvent>& notes, uint8_t num_voices) {
  return [&timeline, &notes, num_voices](Tick tick, uint8_t voice, uint8_t cand_pitch) -> bool {
...
inline std::function<bool(Tick, uint8_t, uint8_t)> makeVerticalSafeWithParallelCheck(
    const HarmonicTimeline& timeline, const std::vector<NoteEvent>& notes, uint8_t num_voices) {
  ...
  for (size_t note_idx = 0; note_idx < notes.size(); ++note_idx) {
    ...
    if (note.voice < num_voices) {
      (*voice_index)[note.voice].push_back(
          {note_idx, note.start_tick, note.start_tick + note.duration});
    }
  }
  ...
  auto pitchAt = [&notes_ref = notes, voice_idx = voice_index](uint8_t voice, Tick tick) -> int {
  ...
  return [&timeline, &notes, num_voices, pitchAt](Tick tick, uint8_t voice,
    ...
    if (prev_tick == tick)
      return true;  // No previous beat data.
    ...
    for (uint8_t other_voice = 0; other_voice < num_voices; ++other_voice) {
      ...
      if (motion_cand != 0 && motion_other != 0 && (motion_cand > 0) == (motion_other > 0)) {
