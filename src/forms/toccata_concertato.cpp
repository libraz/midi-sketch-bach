...
HarmonicTimeline buildConcertatoHarmonicPlan(const KeySignature& key_sig,
  ...
  auto addChord = [&](Tick tick, Tick end_tick, ChordDegree degree, ChordQuality quality,
                      int root_offset, float weight = 1.0f) {
    HarmonicEvent e;
    e.tick = tick;
    e.end_tick = end_tick;
    e.key = key_sig.tonic;
  ...
  // Allegro: I -> III -> V -> I -> IV -> V/V -> V -> I (mediant adds color)
  if (sections.size() >= 1) {
    ...
    addChord(s + seg, s + seg * 2, ChordDegree::iii, iii_q, iii_offset);
    addChord(s + seg * 2, s + seg * 3, ChordDegree::V, ChordQuality::Major, 7);
    addChord(s + seg * 3, s + seg * 4, ChordDegree::I, tonic_q, 0);
    addChord(s + seg * 4, s + seg * 5, ChordDegree::IV,
             minor ? ChordQuality::Minor : ChordQuality::Major, 5);
    addChord(s + seg * 5, s + seg * 6, ChordDegree::V_of_V, ChordQuality::Major, 2);
    addChord(s + seg * 6, s + seg * 7, ChordDegree::V, ChordQuality::Major, 7);
    addChord(s + seg * 7, sections[0].end, ChordDegree::I, tonic_q, 0);
  }
  ...
  if (sections.size() >= 2) {
    ...
    addChord(s, s + seg, ChordDegree::vi, minor ? ChordQuality::Major : ChordQuality::Minor,
             minor ? 8 : 9);
    addChord(s + seg, s + seg * 2, ChordDegree::IV,
             minor ? ChordQuality::Minor : ChordQuality::Major, 5);
    addChord(s + seg * 2, s + seg * 3, ChordDegree::ii, ChordQuality::Minor, 2);
    addChord(s + seg * 3, s + seg * 4, ChordDegree::V, ChordQuality::Major, minor ? 8 : 9);  // V/vi
    addChord(s + seg * 4, s + seg * 5, ChordDegree::vi,
             minor ? ChordQuality::Major : ChordQuality::Minor, minor ? 8 : 9);
    addChord(s + seg * 5, s + seg * 6, ChordDegree::IV,
             minor ? ChordQuality::Minor : ChordQuality::Major, 5);
    addChord(s + seg * 6, s + seg * 7, ChordDegree::V, ChordQuality::Major, 7);
    addChord(s + seg * 7, sections[1].end, ChordDegree::I,
             minor ? ChordQuality::Minor : ChordQuality::Major, 0);
  }

  // Vivace: I -> V/IV -> IV -> VI -> V -> V/V -> V7 -> I (VI adds deceptive color)
  if (sections.size() >= 3) {
    ...
    addChord(s + seg * 2, s + seg * 3, ChordDegree::IV,
             minor ? ChordQuality::Minor : ChordQuality::Major, 5);
    addChord(s + seg * 3, s + seg * 4, ChordDegree::vi,
             minor ? ChordQuality::Major : ChordQuality::Minor,
             minor ? 8 : 9);  // VI in minor = deceptive relation
    addChord(s + seg * 4, s + seg * 5, ChordDegree::V, ChordQuality::Major, 7);
    addChord(s + seg * 5, s + seg * 6, ChordDegree::V_of_V, ChordQuality::Major, 2);
    addChord(s + seg * 6, s + seg * 7, ChordDegree::V, ChordQuality::Dominant7, 7);
    addChord(s + seg * 7, sections[2].end, ChordDegree::I, ChordQuality::Major, 0, 1.5f);
  }
...
std::vector<NoteEvent> generateCantabileMelody(const HarmonicTimeline& timeline,
                                               const KeySignature& key_sig, uint8_t voice,
  ...
  while (tick < end_tick) {
    ...
    if (roll < 0.25f)
      ...
      dur = kDottedQuarter + kQuarterNote;  // dotted half
    else
      dur = kWholeNote;

    if (tick + dur > end_tick)
      dur = end_tick - tick;
    if (dur == 0)
      break;
    ...
    if (beat == 0 || beat == 2) {
      ...
      uint8_t search_low = (current > low + kCantabileRange) ? current - kCantabileRange : low;
      uint8_t search_high = (current + kCantabileRange < high) ? current + kCantabileRange : high;
      ...
      if (ascending) {
        if (idx + step < scale.size())
          idx += step;
        else if (idx >= static_cast<size_t>(step))
          idx -= step;
      } else {
        if (idx >= static_cast<size_t>(step))
          idx -= step;
        else if (idx + step < scale.size())
          idx += step;
      }
    }
    ...
    if (scale[idx] > center + kCantabileRange && idx > 0)
      --idx;
    else if (scale[idx] < center - kCantabileRange && idx + 1 < scale.size())
      ++idx;

    notes.push_back(makeNote(tick, dur, scale[idx], voice, BachNoteSource::ToccataFigure));
    tick += dur;
  }
...
std::vector<NoteEvent> generateMotoPerpetuo_Concertato(const HarmonicTimeline& timeline,
                                                       const KeySignature& key_sig, uint8_t voice,
  ...
  while (tick < end_tick) {
    ...
    if (strong_beat) {
      uint8_t current = scale[idx];
      uint8_t search_low =
          (current > low + kMaxRangeFromCenter) ? current - kMaxRangeFromCenter : low;
      uint8_t search_high =
          (current + kMaxRangeFromCenter < high) ? current + kMaxRangeFromCenter : high;
    ...
    // Soft range boundary.
    uint8_t center = scale[scale.size() / 2];
    if (scale[idx] > center + kMaxRangeFromCenter)
      ascending = false;
    else if (scale[idx] < center - kMaxRangeFromCenter)
      ascending = true;
...
std::vector<NoteEvent> generateSustainedBass(const HarmonicTimeline& timeline,
                                             const KeySignature& /* key_sig */, uint8_t voice,
                                             Tick start_tick, Tick end_tick,
                                             std::mt19937& /* rng */) {
  std::vector<NoteEvent> notes;
...
ToccataResult generateConcertatoToccata(const ToccataConfig& config) {
  ...
  auto sections = buildSectionBoundaries(
      bars, {ToccataSectionId::Allegro, ToccataSectionId::Adagio, ToccataSectionId::Vivace});
  ...
  auto allegro_v0 = generateMotoPerpetuo_Concertato(timeline, config.key, 0, sections[0].start,
                                                    sections[0].end, rng);
  ...
  // Voice 1: eighth-note arpeggios in allegro.
  {
    ...
    while (tick < sections[0].end) {
      ...
      if (!scale1.empty()) {
        all_notes.push_back(makeNote(tick, dur, scale1[idx1], 1, BachNoteSource::ToccataFigure));
        ...
        if (v1_roll < 0.60f) {
          ...
          uint8_t current = scale1[idx1];
          uint8_t best = current;
          int best_dist = 127;
          for (auto ctn : chord_tones) {
            int dist = static_cast<int>(ctn) - static_cast<int>(current);
            bool correct_dir = asc1 ? (dist > 0) : (dist < 0);
            int abs_dist = std::abs(dist);
            if (correct_dir && abs_dist >= 2 && abs_dist < best_dist) {
              best = ctn;
              best_dist = abs_dist;
          ...
          if (best != current) {
            for (size_t sci = 0; sci < scale1.size(); ++sci) {
              if (scale1[sci] >= best) {
                idx1 = sci;
                break;
              }
            ...
            int step = rng::rollRange(rng, 2, 3);
            if (asc1) {
              if (idx1 + step < scale1.size())
                idx1 += step;
              else {
                asc1 = false;
                if (idx1 >= static_cast<size_t>(step))
                  idx1 -= step;
              }
            } else {
              if (idx1 >= static_cast<size_t>(step))
  ...
  auto adagio_melody =
      generateCantabileMelody(timeline, config.key, 1, sections[1].start, sections[1].end, rng);
  ...
  // Voice 0: sustained chord tones during adagio.
  {
    ...
    while (tick < sections[1].end) {
      ...
      Tick dur = kWholeNote;
      if (tick + dur > sections[1].end)
        dur = sections[1].end - tick;
      if (dur == 0)
        break;
      if (!chord_tones.empty()) {
        all_notes.push_back(makeNote(tick, dur, rng::selectRandom(rng, chord_tones), 0,
  ...
  auto vivace_v0 = generateMotoPerpetuo_Concertato(timeline, config.key, 0, sections[2].start,
                                                   sections[2].end, rng);
  all_notes.insert(all_notes.end(), vivace_v0.begin(), vivace_v0.end());

  auto vivace_v1 = generateMotoPerpetuo_Concertato(timeline, config.key, 1, sections[2].start,
                                                   sections[2].end, rng);
  ...
  if (num_voices >= 3) {
    ...
    auto adagio_bass =
        generateSustainedBass(timeline, config.key, 2, sections[1].start, sections[1].end, rng);
  ...
  for (uint8_t vi = 3; vi < num_voices && vi < 5; ++vi) {
    ...
    while (tick < sections[2].end) {
      ...
      if (dur == 0)
        break;
      if (!chord_tones.empty()) {
        all_notes.push_back(makeNote(tick, dur, rng::selectRandom(rng, chord_tones), vi,
                                     BachNoteSource::ToccataGesture));
      }
      tick += dur;
    }
  ...
  assert(countUnknownSource(all_notes) == 0 && "All notes should have source set by generators");
  {
    ...
    ScaleType conc_scale = config.key.is_minor ? ScaleType::HarmonicMinor : ScaleType::Major;
    finalizeFormNotes(all_notes, num_voices, voice_range, config.key.tonic, conc_scale,
                      /*max_consecutive=*/2);
  }
  ...
  Tick vivace_last_2 =
      sections[2].end > 2 * kTicksPerBar ? sections[2].end - 2 * kTicksPerBar : sections[2].start;
