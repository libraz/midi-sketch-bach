...
HarmonicTimeline buildPerpetuusHarmonicPlan(const KeySignature& key_sig,
  ...
  if (sections.size() >= 1) {
    ...
    addChord(s, s + seg, ChordDegree::I, minor ? ChordQuality::Minor : ChordQuality::Major, 0);
    addChord(s + seg, s + seg * 2, ChordDegree::iii,
    ...
    addChord(s + seg * 5, sections[0].end, ChordDegree::I,
  ...
  if (sections.size() >= 2) {
    ...
    addChord(s, s + seg, ChordDegree::I, minor ? ChordQuality::Minor : ChordQuality::Major, 0);
    addChord(s + seg, s + seg * 2, ChordDegree::vi,
             minor ? ChordQuality::Major : ChordQuality::Minor,
             minor ? 8 : 9);  // VI in minor = deceptive relation
    addChord(s + seg * 2, s + seg * 3, ChordDegree::IV,
             minor ? ChordQuality::Minor : ChordQuality::Major, 5);
    addChord(s + seg * 3, s + seg * 4, ChordDegree::iii,
             minor ? ChordQuality::Major : ChordQuality::Minor,
             minor ? 3 : 4);  // III in minor = relative major
    ...
    addChord(s + seg * 6, s + seg * 7, ChordDegree::viiDim, ChordQuality::Diminished7, 11);
  ...
  if (sections.size() >= 3) {
    ...
    addChord(s, s + seg, ChordDegree::IV, minor ? ChordQuality::Minor : ChordQuality::Major, 5);
    addChord(s + seg, s + seg * 2, ChordDegree::V_of_V, ChordQuality::Major, 2);
    addChord(s + seg * 2, s + seg * 3, ChordDegree::V, ChordQuality::Dominant7, 7);
    addChord(s + seg * 3, sections[2].end, ChordDegree::I, ChordQuality::Major, 0, 1.5f);
  }
...
std::vector<NoteEvent> generateMotoPerpetuo(const HarmonicTimeline& timeline,
...
std::vector<NoteEvent> generateChordPad(const HarmonicTimeline& timeline,
                                        const KeySignature& /* key_sig */, uint8_t voice,
                                        Tick start_tick, Tick end_tick, std::mt19937& rng) {
  std::vector<NoteEvent> notes;
  uint8_t low = getToccataLowPitch(voice);
  ...
  while (tick < end_tick) {
    ...
    notes.push_back(makeNote(tick, dur, best, voice, BachNoteSource::ToccataFigure));
    prev_pitch = best;
    tick += dur;
  }
...
ToccataResult generatePerpetuusToccata(const ToccataConfig& config) {
  ...
  auto sections = buildSectionBoundaries(
      bars, {ToccataSectionId::Ascent, ToccataSectionId::Plateau, ToccataSectionId::Climax});
  ...
  auto moto_ascent =
      generateMotoPerpetuo(timeline, config.key, 0, sections[0].start, sections[0].end, rng);
  ...
  Tick v1_entry = sections[0].start + (sections[0].end - sections[0].start) * 40 / 100;
  auto pad_ascent = generateChordPad(timeline, config.key, 1, v1_entry, sections[0].end, rng);
  ...
  auto moto_plat0 =
      generateMotoPerpetuo(timeline, config.key, 0, sections[1].start, sections[1].end, rng);
  all_notes.insert(all_notes.end(), moto_plat0.begin(), moto_plat0.end());

  auto moto_plat1 =
      generateMotoPerpetuo(timeline, config.key, 1, sections[1].start, sections[1].end, rng);
  all_notes.insert(all_notes.end(), moto_plat1.begin(), moto_plat1.end());

  // --- Climax: All manual voices moto perpetuo + pedal ---
  auto moto_clim0 =
      generateMotoPerpetuo(timeline, config.key, 0, sections[2].start, sections[2].end, rng);
  all_notes.insert(all_notes.end(), moto_clim0.begin(), moto_clim0.end());

  auto moto_clim1 =
      generateMotoPerpetuo(timeline, config.key, 1, sections[2].start, sections[2].end, rng);
  ...
  if (num_voices >= 3) {
    ...
    auto pedal_plat =
        generateChordPad(timeline, config.key, 2, sections[1].start, sections[1].end, rng);
    // Remap to pedal source.
    for (auto& n : pedal_plat)
      n.source = BachNoteSource::PedalPoint;
    all_notes.insert(all_notes.end(), pedal_plat.begin(), pedal_plat.end());

    auto pedal_clim =
        generateChordPad(timeline, config.key, 2, sections[2].start, sections[2].end, rng);
    for (auto& n : pedal_clim)
      n.source = BachNoteSource::PedalPoint;
  ...
  for (uint8_t vi = 3; vi < num_voices && vi < 5; ++vi) {
    auto extra =
        generateChordPad(timeline, config.key, vi, sections[2].start, sections[2].end, rng);
  ...
  auto applyOrnamentsToSection = [&](std::vector<NoteEvent>& section_notes, float density,
                                     VoiceRole role, const std::vector<Tick>& cadence_ticks) {
    OrnamentContext ctx;
  ...
  assert(countUnknownSource(all_notes) == 0 && "All notes should have source set by generators");
  {
    ...
    ScaleType perp_scale = config.key.is_minor ? ScaleType::HarmonicMinor : ScaleType::Major;
    finalizeFormNotes(all_notes, num_voices, voice_range, config.key.tonic, perp_scale,
                      /*max_consecutive=*/2);
  }
  ...
  Tick plat_mid = sections[1].start + (sections[1].end - sections[1].start) / 2;
