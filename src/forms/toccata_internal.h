...
inline float computeSustainedEnergy(const std::vector<NoteEvent>& notes, Tick window_start,
                                    Tick window_end) {
  float energy = 0.0f;
...
inline bool isValidGrandPause(Tick gap_start, Tick gap_duration, ToccataStyleMode style_mode,
                              const std::vector<NoteEvent>& notes) {
  // Only valid in Phantasticus or Recitativo style.
  if (style_mode != ToccataStyleMode::Phantasticus && style_mode != ToccataStyleMode::Recitativo) {
    return false;
  }
  ...
  if (pre_energy < kMinPreGapEnergy)
    return false;
  ...
  for (const auto& n : notes) {
    if (n.start_tick >= post_start && n.start_tick < post_start + kTicksPerBeat) {
...
inline std::vector<Tick> allocateBars(int total_bars, const std::vector<float>& proportions) {
...
inline std::vector<Tick> allocateBarsDiscrete(
    int total_bars, const std::map<int, std::vector<Tick>>& discrete_tables,
    const std::vector<float>& fallback_proportions, const std::vector<Tick>& min_bars_per_section) {
  ...
  if (total_bars <= 0 || min_total >= static_cast<Tick>(total_bars)) {
    ...
    for (size_t idx = 0; idx < num; ++idx) {
      ...
      bars[idx] =
          std::max(static_cast<Tick>(1), static_cast<Tick>(static_cast<float>(min_b) * scale));
      assigned += bars[idx];
    }
...
inline std::vector<ToccataSectionBoundary> buildSectionBoundaries(
    const std::vector<Tick>& bar_counts, const std::vector<ToccataSectionId>& ids) {
