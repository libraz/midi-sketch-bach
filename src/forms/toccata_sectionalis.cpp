...
HarmonicTimeline buildSectionalisHarmonicPlan(const KeySignature& key_sig,
  ...
  auto segmentChords = [&](Tick s, Tick e, int count,
    ...
    for (int i = 0; i < count && i < static_cast<int>(chords.size()); ++i) {
      ...
      addChord(cs, ce, std::get<0>(chords[i]), std::get<1>(chords[i]), std::get<2>(chords[i]));
    }
  };
  ...
  if (sections.size() >= 2) {
    segmentChords(
        sections[1].start, sections[1].end, 6,
        {
            {ChordDegree::I, tonic_q, 0},
            {ChordDegree::V, ChordQuality::Major, 7},
            {ChordDegree::vi, minor ? ChordQuality::Major : ChordQuality::Minor, minor ? 8 : 9},
            {ChordDegree::IV, sub_q, 5},
            {ChordDegree::V, ChordQuality::Major, 7},
            {ChordDegree::I, tonic_q, 0},
        });
  ...
  // Cadenza: V pedal
  if (sections.size() >= 4) {
    addChord(sections[3].start, sections[3].end, ChordDegree::V, ChordQuality::Major, 7, 0.8f);
  }
...
std::vector<NoteEvent> generateQuasiFugalSection(const HarmonicTimeline& /* timeline */,
  ...
  for (uint8_t v = 0; v < voices; ++v) {
    ...
    for (int i = 0; i < kMotifNotes; ++i) {
      ...
      notes.push_back(makeNote(tick, dur, scale[new_idx], v, BachNoteSource::ToccataFigure));
    }

    // Free counterpoint after motif.
    Tick free_start = entry_tick + kMotifDuration;
    if (free_start >= end_tick)
      continue;
    ...
    while (tick < end_tick) {
      ...
      notes.push_back(makeNote(tick, dur, scale[idx], v, BachNoteSource::ToccataFigure));
...
std::vector<NoteEvent> generatePedalCadenza(const HarmonicTimeline& /* timeline */,
  ...
  // Phase 1: descending scale run.
  {
    ...
    while (tick < cadenza_mid && tick < end_tick) {
      ...
      if (dur == 0)
        break;
      notes.push_back(makeNote(tick, dur, scale[idx], kPedalVoice, BachNoteSource::PedalPoint));
      tick += dur;
      if (idx > 0)
        --idx;
      else
        idx = scale.size() > 1 ? scale.size() - 1 : 0;
    }
  }
  ...
  // Phase 2: dominant arpeggio + trill.
  {
    ...
    for (size_t ai = 0; tick < arp_end && tick < end_tick; ++ai) {
      ...
      if (dur == 0)
        break;
      notes.push_back(makeNote(tick, dur, p, kPedalVoice, BachNoteSource::PedalPoint));
      tick += dur;
    }
    ...
    while (tick < end_tick) {
      ...
      if (dur == 0)
        break;
      notes.push_back(makeNote(tick, dur, use_main ? dom_pitch : upper_pitch, kPedalVoice,
...
std::vector<NoteEvent> generateFreeSection(const HarmonicTimeline& timeline,
                                           const KeySignature& key_sig, uint8_t num_voices,
...
ToccataResult generateSectionalisToccata(const ToccataConfig& config) {
  ...
  auto bars = allocateBars(config.total_bars, {0.20f, 0.25f, 0.20f, 0.15f, 0.20f});
  auto sections = buildSectionBoundaries(
      bars, {ToccataSectionId::Free1, ToccataSectionId::QuasiFugal, ToccataSectionId::Free2,
             ToccataSectionId::Cadenza, ToccataSectionId::Coda});
  ...
  auto free1 = generateFreeSection(timeline, config.key, num_voices, sections[0].start,
                                   sections[0].end, rng);
  all_notes.insert(all_notes.end(), free1.begin(), free1.end());

  // --- QuasiFugal: Imitative texture ---
  auto quasi = generateQuasiFugalSection(timeline, config.key, num_voices, sections[1].start,
                                         sections[1].end, rng);
  all_notes.insert(all_notes.end(), quasi.begin(), quasi.end());

  // --- Free2: Another free section with chromatic color ---
  auto free2 = generateFreeSection(timeline, config.key, num_voices, sections[2].start,
                                   sections[2].end, rng);
  all_notes.insert(all_notes.end(), free2.begin(), free2.end());

  // --- Cadenza: Pedal solo ---
  auto cadenza =
      generatePedalCadenza(timeline, config.key, sections[3].start, sections[3].end, rng);
  all_notes.insert(all_notes.end(), cadenza.begin(), cadenza.end());

  // --- Coda: All voices, driving to final cadence ---
  auto coda = generateFreeSection(timeline, config.key, num_voices, sections[4].start,
                                  sections[4].end, rng);
  ...
  // Constraint-driven finalize: overlap dedup + range clamping + repeat mitigation.
  {
    ...
    ScaleType sect_scale = config.key.is_minor ? ScaleType::HarmonicMinor : ScaleType::Major;
    finalizeFormNotes(all_notes, num_voices, voice_range, config.key.tonic, sect_scale,
                      /*max_consecutive=*/2);
  }
