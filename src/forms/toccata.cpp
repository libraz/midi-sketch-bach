...
constexpr ToccataSectionId kDramaticusPhaseIds[8] = {
    ToccataSectionId::Gesture,        ToccataSectionId::EchoCollapse,
    ToccataSectionId::RecitExpansion, ToccataSectionId::SequenceClimb1,
    ToccataSectionId::HarmonicBreak,  ToccataSectionId::SequenceClimb2,
    ToccataSectionId::DomObsession,   ToccataSectionId::FinalExplosion,
};

// Energy curve (two-peak arch): A=0.90 B=0.30 C=0.50 D=0.75 E=0.35 F=0.85 G=0.95 H=1.00
constexpr float kDramaticusEnergy[8] = {0.90f, 0.30f, 0.50f, 0.75f, 0.35f, 0.85f, 0.95f, 1.00f};

// Range ceiling for voice 0 (Great) per phase -- staircase release.
constexpr uint8_t kDramaticusRangeCeiling[8] = {84, 84, 86, 89, 84, 93, 96, 96};

// Registration velocity hints per phase (non-monotonic).
constexpr uint8_t kDramaticusVelocity[8] = {100, 45, 60, 85, 50, 95, 105, 115};

// Ornament density per phase.
constexpr float kDramaticusOrnament[8] = {0.12f, 0.04f, 0.10f, 0.08f, 0.03f, 0.08f, 0.06f, 0.10f};
...
constexpr float kDramaticusFallbackProportions[8] = {0.083f, 0.083f, 0.167f, 0.125f,
                                                     0.083f, 0.167f, 0.167f, 0.125f};
...
std::vector<NoteEvent> generateFigured(const HarmonicTimeline& timeline,
                                       const KeySignature& key_sig,
                                       int phase_idx,  // 0-7 for phases A-H
                                       uint8_t voice,  // 0 or 1
                                       Tick start_tick, Tick end_tick, std::mt19937& rng) {
  ...
  uint8_t ceiling =
      (voice < 2) ? profile.voice_ceiling[voice] : toccata_internal::getToccataHighPitch(voice);
  ...
  auto result = generateFigurationSpan(ctx, start_tick, end_tick, voice, timeline, rng);
...
uint8_t getToccataLowPitch(uint8_t voice_idx) {
  switch (voice_idx) {
    ...
    case 1:
      return 52;  // E3 (Swell)
    case 2:
      return organ_range::kPedalLow;  // 24 (Pedal unchanged)
    case 3:
      return 43;  // G2 (Positiv)
    default:
      return 52;
  }
}
...
uint8_t getToccataHighPitch(uint8_t voice_idx) {
  switch (voice_idx) {
    ...
    case 1:
      return 76;  // E5 (Swell)
    case 2:
      return organ_range::kPedalHigh;  // 50 (Pedal unchanged)
    case 3:
      return 67;  // G4 (Positiv)
    default:
      return 76;
  }
}
...
HarmonicTimeline buildDramaticusHarmonicPlan(const KeySignature& key_sig,
  ...
  auto addChord = [&](Tick tick, Tick end_tick, ChordDegree degree, ChordQuality quality,
                      int root_offset, float weight = 1.0f, float chord_bias = 0.6f) {
    HarmonicEvent evt;
    evt.tick = tick;
    ...
    evt.bass_pitch =
        clampPitch(36 + (tpc + root_offset) % 12, organ_range::kPedalLow, organ_range::kPedalHigh);
    evt.weight = weight;
  ...
  // A: Gesture -- V -> i (BWV565-style V opening)
  {
    ...
    addChord(mid, seg_end, ChordDegree::I, minor ? ChordQuality::Minor : ChordQuality::Major, 0);
  }

  // B: EchoCollapse -- i sustained
  addChord(phases[1].start, phases[1].end, ChordDegree::I,
           minor ? ChordQuality::Minor : ChordQuality::Major, 0, 0.5f);
  ...
  // C: RecitExpansion -- i -> III -> viio7 -> V -> VI (mediant before diminished)
  {
    ...
    addChord(seg_start, t1, ChordDegree::I, minor ? ChordQuality::Minor : ChordQuality::Major, 0);
    addChord(t1, t2, ChordDegree::iii, minor ? ChordQuality::Major : ChordQuality::Minor,
    ...
    addChord(t4, seg_end, ChordDegree::vi, minor ? ChordQuality::Major : ChordQuality::Minor,
  ...
  // D: SequenceClimb1 -- iv -> vi -> V/V -> V (vi adds modal color)
  {
    ...
    addChord(seg_start, t1, ChordDegree::IV, minor ? ChordQuality::Minor : ChordQuality::Major, 5);
    addChord(t1, t2, ChordDegree::vi, minor ? ChordQuality::Major : ChordQuality::Minor,
  ...
  // F: SequenceClimb2 -- III -> iv -> V/V -> V7 (III opens with mediant color)
  {
    ...
    Tick t2 = seg_start + dur * 2 / 4;
    Tick t3 = seg_start + dur * 3 / 4;
    addChord(seg_start, t1, ChordDegree::iii, minor ? ChordQuality::Major : ChordQuality::Minor,
             minor ? 3 : 4);  // III in minor = relative major
    addChord(t1, t2, ChordDegree::IV, minor ? ChordQuality::Minor : ChordQuality::Major, 5);
    addChord(t2, t3, ChordDegree::V_of_V, ChordQuality::Major, 2);
  ...
  // G: DomObsession -- V -> viio7/V -> V -> V7 -> V sustained
  {
    ...
    addChord(t1, t2, ChordDegree::viiDim, ChordQuality::Diminished7, 6, 1.0f, 0.9f);  // viio7/V
...
Tick generateMordentWave(uint8_t start_pitch, const std::vector<uint8_t>& scale_tones,
                         Tick start_tick, Tick section_end, std::vector<NoteEvent>& notes,
                         uint8_t low0, uint8_t high0, uint8_t low1, uint8_t high1) {
  if (scale_tones.empty() || start_tick >= section_end)
    return start_tick;
  ...
  for (int i = 0; i < 3 && tick < section_end; ++i) {
    ...
    if (dur == 0)
      break;
    notes.push_back(makeNote(tick, dur, mordent_pitches0[i], 0, BachNoteSource::ToccataGesture));
    notes.push_back(makeNote(tick, dur, mordent_pitches1[i], 1, BachNoteSource::ToccataGesture));
    tick += dur;
  }
  if (tick >= section_end)
    return tick;
...
void generateBlockChordNotes(const KeySignature& key_sig, Tick tick, Tick end_tick,
  ...
  uint8_t tpc = static_cast<uint8_t>(key_sig.tonic);
  uint8_t third = clampPitch(60 + tpc + (key_sig.is_minor ? 3 : 4), getToccataLowPitch(1),
                             getToccataHighPitch(1));
  uint8_t fifth = clampPitch(60 + tpc + 7, getToccataLowPitch(0), getToccataHighPitch(0));
  ...
  if (num_voices >= 3) {
    uint8_t bass = clampPitch(36 + tpc, getToccataLowPitch(2), getToccataHighPitch(2));
...
[[maybe_unused]]
void generateArpeggioFlourish(const HarmonicTimeline& timeline, Tick start_tick, Tick end_tick,
                              std::vector<NoteEvent>& notes, std::mt19937& rng) {
  ...
  while (tick < end_tick) {
    ...
    notes.push_back(makeNote(tick, dur, chord_tones[idx], 0, BachNoteSource::ToccataGesture));
...
std::vector<NoteEvent> generateOpeningGesture(const HarmonicTimeline& timeline,
                                              const KeySignature& key_sig, Tick start_tick,
                                              Tick end_tick, std::mt19937& rng) {
  ...
  for (size_t i = 0; i < scale_tones0.size(); ++i) {
    if (scale_tones0[i] <= start_pitch1)
      dom_idx = i;
  }
  uint8_t start_pitch2 = (dom_idx > 0) ? scale_tones0[dom_idx - 1] : scale_tones0[0];

  Tick tick = start_tick;

  if (opening_bars >= 4) {
    // Full pattern: wave1 + pause + wave2 + pause + block chord + arpeggio
    tick = generateMordentWave(start_pitch1, scale_tones0, tick, end_tick, notes, low0, high0, low1,
                               high1);
    tick = std::min(tick + kGrandPauseDuration, end_tick);

    if (tick < end_tick - kTicksPerBar * 2) {
      tick = generateMordentWave(start_pitch2, scale_tones0, tick, end_tick, notes, low0, high0,
                                 low1, high1);
      tick = std::min(tick + kGrandPauseDuration, end_tick);
    }
...
Tick selectRecitDuration(std::mt19937& rng) {
  ...
  if (roll < 0.10f)
    return kThirtySecondNote;
  if (roll < 0.30f)
    return kSixteenthNote;
  if (roll < 0.60f)
    return kEighthNote;
  if (roll < 0.80f)
    return kQuarterNote;
  if (roll < 0.90f)
    return kDottedQuarter;
  return kHalfNote;
}
...
int selectRecitScaleStep(std::mt19937& rng, bool ascending) {
  ...
  int magnitude;
  if (roll < 0.50f)
    magnitude = 1;  // Step (2nd)
  else if (roll < 0.80f)
    magnitude = rng::rollRange(rng, 2, 3);  // Skip (3rd/4th)
  else
    magnitude = 4;  // Max leap (5th)
...
ClimbingMotif extractClimbingMotif(const std::vector<NoteEvent>& opening_notes, uint8_t voice,
                                   Tick max_duration = 0) {
  ClimbingMotif motif;
  ...
  for (const auto& note : opening_notes) {
    ...
    int8_t rel = static_cast<int8_t>(static_cast<int>(note.pitch) - static_cast<int>(base_pitch));
    motif.intervals.push_back(rel);
    motif.durations.push_back(note.duration);
    ++motif.num_notes;
    if (motif.num_notes >= kMaxMotifNotes)
      break;
  }
...
static bool isFigureDescending(const MelodicFigure& fig) {
  if (!fig.degree_intervals || fig.note_count < 2)
    return false;
  int sum = 0;
...
static ClimbingMotif tryVocabularyGesture(const ClimbingMotif& motif, ToccataArchetype archetype,
                                          int current_base_pitch, int next_base_pitch, Key key,
                                          ScaleType scale, std::mt19937& rng_engine) {
  auto hint = getArchetypeFigures(archetype);
  if (!rng::rollProbability(rng_engine, hint.activation_prob))
    return motif;
  ...
  for (int fig_idx = 0; fig_idx < hint.count; ++fig_idx) {
    if (!hint.figures[fig_idx])
      continue;
    const MelodicFigure& fig = *hint.figures[fig_idx];

    // Register-linked direction check.
    if (isFigureDescending(fig) && !register_descending)
      continue;
    if (isFigureAscending(fig) && register_descending)
      continue;

    // Only apply if note counts are compatible.
    if (fig.note_count != motif.num_notes)
      continue;
    if (!fig.degree_intervals)
      continue;
    ...
    int abs_deg =
        scale_util::pitchToAbsoluteDegree(clampPitch(current_base_pitch, 0, 127), key, scale);
    ...
    for (uint8_t note_idx = 1; note_idx < fig.note_count; ++note_idx) {
      ...
      result.intervals[note_idx] =
          static_cast<int8_t>(static_cast<int>(pitch) - static_cast<int>(base_p));
    }
    return result;
  }
...
std::vector<NoteEvent> generateClimbFromMotif(const ClimbingMotif& motif, uint8_t base_pitch,
                                              uint8_t voice, Tick start_tick, Tick max_end,
  ...
  for (uint8_t idx = 0; idx < motif.num_notes && tick < max_end; ++idx) {
    ...
    notes.push_back(makeNote(tick, dur, clamped, voice, BachNoteSource::ToccataFigure));
    tick += dur;
  }
...
std::vector<NoteEvent> generateSequenceClimb(const HarmonicTimeline& timeline,
                                             const KeySignature& key_sig,
                                             const ClimbingMotif& motif, Tick start_tick,
                                             Tick end_tick, uint8_t range_ceiling, int phase_idx,
                                             std::mt19937& rng) {
  std::vector<NoteEvent> notes;
  ...
  for (size_t idx = 0; idx < scale_tones.size(); ++idx) {
    int dist = std::abs(static_cast<int>(scale_tones[idx]) - static_cast<int>(target_start));
  ...
  for (int stmt = 0; stmt < num_statements && tick < end_tick; ++stmt) {
    ...
    auto motif_notes = generateClimbFromMotif(motif, base_pitch, 0, tick, stmt_end, low0, high0);
    ...
    for (const auto& mote : motif_notes) {
      ...
      if (note_end > motif_end_tick)
        motif_end_tick = note_end;
    }
    ...
    // Sparse texture: one note per bar at beat 0, creating breathing room.
    {
      ...
      uint8_t support_target = clampPitch(static_cast<int>(base_pitch) - 7, low1, high1);
      ...
      while (v1_tick < motif_end_tick) {
        ...
        notes.push_back(makeNote(v1_tick, dur, support_pitch, 1, BachNoteSource::FreeCounterpoint));
    ...
    if (stmt < num_statements - 1 && tick < end_tick) {
      ...
      if (connector_dur >= kTicksPerBeat) {
        ...
        auto fig_result = generateFigurationSpan(fig_ctx, tick, connector_end, 0, timeline, rng);

        if (!fig_result.notes.empty()) {
          notes.insert(notes.end(), fig_result.notes.begin(), fig_result.notes.end());
          ...
          if (bridge_dur > 0) {
            uint8_t bridge_pitch = clampPitch(static_cast<int>(motif_end_pitch) + 1, low0, high0);
            ...
            notes.push_back(
                makeNote(tick, bridge_dur, bridge_pitch, 0, BachNoteSource::ToccataGesture));
            tick += bridge_dur;
          }
        }
        ...
        for (int conn = 0; conn < num_conn && tick < connector_end; ++conn) {
          if (conn_idx + 1 < scale_tones.size())
            ++conn_idx;
          Tick dur = std::min(kEighthNote, connector_end - tick);
          if (dur == 0)
            break;
          notes.push_back(
              makeNote(tick, dur, scale_tones[conn_idx], 0, BachNoteSource::ToccataGesture));
          tick += dur;
        }
      }
...
std::vector<NoteEvent> generateEchoCollapse(const HarmonicTimeline& timeline,
  ...
  if (chord_tones_v1.empty() || chord_tones_v0.empty())
    return notes;
  ...
  // Single-voice texture for the echo collapse -- dying away to near-silence.
  {
    ...
    while (tick < sustain_end) {
      ...
      size_t idx = rng::rollRange(rng, 0, static_cast<int>(chord_tones_v1.size()) - 1);
      notes.push_back(makeNote(tick, dur, chord_tones_v1[idx], 1, BachNoteSource::ToccataGesture));
  ...
  // Sub-section 3 (~30%): fading to near-silence, 1-2 soft whole notes.
  {
    ...
    for (int fi = 0; fi < fade_count && tick < end_tick; ++fi) {
      ...
      notes.push_back(makeNote(tick, dur, fade_pitch, 1, BachNoteSource::ToccataGesture));
      tick += dur;
...
std::vector<NoteEvent> generateRecitativeExpansion(const HarmonicTimeline& timeline,
  ...
  for (Tick bar = 0; bar < phase_bars; ++bar) {
    ...
    {
      auto fig = generateFigured(timeline, key_sig, 2, melody_voice, bar_start, bar_end, rng);
      notes.insert(notes.end(), fig.begin(), fig.end());
    }
    ...
    while (tick < bar_end) {
      ...
      notes.push_back(makeNote(tick, dur, rng::selectRandom(rng, valid), chord_voice,
                               BachNoteSource::FreeCounterpoint));
      tick += dur;
    }
  }
  ...
  if (dim_end > start_tick) {
    ...
    while (tick < dim_end) {
      ...
      if (!tones_v0.empty()) {
        ...
        notes.push_back(makeNote(tick, note_dur, tones_v0[idx], 0, BachNoteSource::ToccataGesture));
      }

      if (!tones_v1.empty()) {
        size_t idx = 0;
        if (tones_v1.size() > 1) {
          idx = rng::rollRange(rng, 0, static_cast<int>(tones_v1.size() / 2));
        }
        notes.push_back(makeNote(tick, note_dur, tones_v1[idx], 1, BachNoteSource::ToccataGesture));
      }
  ...
  if (pause_end < end_tick) {
    ...
    if (final_bar_start >= pause_end && final_bar_start < end_tick) {
      ...
      if (!dom_tones_v1.empty()) {
        ...
        if (sustain_dur > 0) {
          notes.push_back(makeNote(final_bar_start, sustain_dur, dom_tones_v1[0], 1,
...
std::vector<NoteEvent> generateDominantObsession(const HarmonicTimeline& timeline,
                                                 const KeySignature& key_sig, uint8_t num_voices,
  ...
  // Single voice sustain narrows texture toward the cadence.
  {
    ...
    while (tick < end_tick) {
      ...
      if (dur == 0)
        break;
      notes.push_back(makeNote(tick, dur, sustained0, 0, BachNoteSource::ToccataGesture));
      tick += dur;
    }
  }
  ...
  // Sub-phase 3: final I Major block chord (Picardy third).
  {
    ...
    while (tick < end_tick) {
      ...
      if (dur == 0)
        break;
      notes.push_back(makeNote(tick, dur, fifth, 0, BachNoteSource::ToccataGesture));
      notes.push_back(makeNote(tick, dur, major_third, 1, BachNoteSource::ToccataGesture));
      tick += dur;
    }

    if (num_voices >= 3) {
      uint8_t bass_root = clampPitch(36 + tpc, organ_range::kPedalLow, organ_range::kPedalHigh);
      Tick pedal_tick = trans_end;
      while (pedal_tick < end_tick) {
        Tick dur = std::min(kWholeNote, end_tick - pedal_tick);
        if (dur == 0)
          break;
        notes.push_back(makeNote(pedal_tick, dur, bass_root, 2, BachNoteSource::PedalPoint));
        pedal_tick += dur;
      }
    }
  }
...
std::vector<NoteEvent> generatePhasePedal(const HarmonicTimeline& timeline,
                                          const KeySignature& key_sig, int phase_idx,
                                          Tick start_tick, Tick end_tick, std::mt19937& /*rng*/) {
  std::vector<NoteEvent> notes;
  if (start_tick >= end_tick)
    return notes;
  if (phase_idx < 0 || phase_idx > 7)
    return notes;

  uint8_t tpc = static_cast<uint8_t>(key_sig.tonic);
  uint8_t tonic = clampPitch(36 + tpc, organ_range::kPedalLow, organ_range::kPedalHigh);
  uint8_t dominant = clampPitch(36 + static_cast<int>(getDominant(key_sig).tonic),
                                organ_range::kPedalLow, organ_range::kPedalHigh);
  uint8_t subdominant = clampPitch(36 + static_cast<int>(getSubdominant(key_sig).tonic),
                                   organ_range::kPedalLow, organ_range::kPedalHigh);
  ...
  if (phase_idx == 0) {
    ...
    while (tick < end_tick) {
      const HarmonicEvent& evt = timeline.getAt(tick);
      uint8_t bass = clampPitch(static_cast<int>(evt.bass_pitch), organ_range::kPedalLow,
  ...
  if (energy >= 0.85f) {
    ...
    while (tick < end_tick) {
      ...
      const HarmonicEvent& evt = timeline.getAt(tick);
      uint8_t timeline_bass = clampPitch(static_cast<int>(evt.bass_pitch), organ_range::kPedalLow,
                                         organ_range::kPedalHigh);
...
ToccataResult generateDramaticusToccata(const ToccataConfig& config) {
  ...
  std::vector<float> fallback_props(std::begin(kDramaticusFallbackProportions),
                                    std::end(kDramaticusFallbackProportions));
  std::vector<Tick> min_bars(std::begin(kDramaticusMinBars), std::end(kDramaticusMinBars));

  auto phase_bars = toccata_internal::allocateBarsDiscrete(effective_bars, kDramaticusBarsTables,
                                                           fallback_props, min_bars);

  // --- 2. Phase boundaries ---
  std::vector<ToccataSectionId> phase_ids(std::begin(kDramaticusPhaseIds),
                                          std::end(kDramaticusPhaseIds));
  ...
  // --- 3. Harmonic plan ---
  HarmonicTimeline timeline = buildDramaticusHarmonicPlan(config.key, phases);

  // --- 4. Phase generation (rng consumed in deterministic A-H order) ---
  // A: Gesture
  auto a_notes = generateOpeningGesture(timeline, config.key, phases[0].start, phases[0].end, rng);

  // B: EchoCollapse
  auto b_notes = generateEchoCollapse(timeline, config.key, phases[1].start, phases[1].end, rng);

  // C: RecitExpansion
  auto c_notes =
      generateRecitativeExpansion(timeline, config.key, phases[2].start, phases[2].end, rng);
  ...
  motif = tryVocabularyGesture(motif, config.archetype, static_cast<int>(getToccataLowPitch(0)),
                               static_cast<int>(kDramaticusRangeCeiling[3]), config.key.tonic,
                               climb_scale, rng);
  auto d_notes = generateSequenceClimb(timeline, config.key, motif, phases[3].start, phases[3].end,
                                       kDramaticusRangeCeiling[3], 3, rng);

  // E: HarmonicBreak
  auto e_notes = generateHarmonicBreak(timeline, config.key, phases[4].start, phases[4].end, rng);
  ...
  auto f_notes = generateSequenceClimb(timeline, config.key, motif_f, phases[5].start,
                                       phases[5].end, kDramaticusRangeCeiling[5], 5, rng);

  // G: DomObsession
  auto g_notes = generateDominantObsession(timeline, config.key, num_voices, phases[6].start,
                                           phases[6].end, rng);

  // H: FinalExplosion
  auto h_notes =
      generateFinalExplosion(timeline, config.key, num_voices, phases[7].start, phases[7].end, rng);
  ...
  PhaseRef phase_refs[8] = {
      {&a_notes, VoiceRole::Propel}, {&b_notes, VoiceRole::Respond}, {&c_notes, VoiceRole::Respond},
      {&d_notes, VoiceRole::Propel}, {&e_notes, VoiceRole::Respond}, {&f_notes, VoiceRole::Propel},
  ...
  if (num_voices >= 3) {
    for (int i = 0; i < 8; ++i) {
      auto pedal = generatePhasePedal(timeline, config.key, i, phases[i].start, phases[i].end, rng);
  ...
  // --- 9. Constraint-driven finalize ---
  {
    ...
    ScaleType toc_scale = config.key.is_minor ? ScaleType::HarmonicMinor : ScaleType::Major;
    finalizeFormNotes(all_notes, num_voices, voice_range, config.key.tonic, toc_scale,
                      /*max_consecutive=*/2);
  }
