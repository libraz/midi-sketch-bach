...
Feasibility FeasibilityEstimator::estimate(const CounterpointState& state,
                                           const IRuleEvaluator& rules,
                                           const CollisionResolver& resolver, VoiceId voice_id,
                                           Tick tick, Tick duration, const InvariantSet& invariants,
  ...
  for (uint8_t p = range->low; p <= range->high && total_tried < kMaxCandidates; p += stride) {
    total_tried++;

    // Check if pitch is safe via collision resolver.
    bool safe = resolver.isSafeToPlace(state, rules, voice_id, p, tick, duration);
    if (!safe)
      continue;

    // Check Hard invariants.
    auto inv_check =
        invariants.satisfies(p, voice_id, tick, snap, &rules, nullptr, &state, nullptr, 0);
    if (inv_check.hard_violations > 0)
      continue;
  ...
  float scale =
      static_cast<float>(range_size) / static_cast<float>(total_tried > 0 ? total_tried : 1);
  result.min_choices = static_cast<uint16_t>(valid_count);
  result.max_choices =
      static_cast<uint16_t>(std::min(static_cast<int>(valid_count * scale), 65535));

  return result;
}

Feasibility FeasibilityEstimator::estimateWithCascade(const CounterpointState& state,
                                                      const IRuleEvaluator& rules,
  ...
  for (uint8_t p = range->low; p <= range->high && total_tried < kMaxCandidates; p += stride) {
    ...
    PlacementResult placement = resolver.findSafePitch(state, rules, voice_id, p, tick, duration);
    if (!placement.accepted)
      continue;

    // Check Hard invariants on the cascade's chosen pitch.
    auto inv_check = invariants.satisfies(placement.pitch, voice_id, tick, snap, &rules, nullptr,
                                          &state, nullptr, 0);
    if (inv_check.hard_violations > 0)
      continue;
