...
float SubjectScore::composite() const {
  ...
  return interval_variety * 0.15f + rhythm_diversity * 0.10f + contour_balance * 0.15f +
         range_score * 0.10f + step_motion_ratio * 0.25f + tonal_stability * 0.15f +
...
SubjectScore SubjectValidator::evaluate(const Subject& subject) const {
  ...
  if (subject.noteCount() >= 2) {
    ...
    for (size_t idx = 1; idx < subject.notes.size(); ++idx) {
      int abs_interval = absoluteInterval(subject.notes[idx].pitch, subject.notes[idx - 1].pitch);
      total_intervals++;
      if (abs_interval >= 1 && abs_interval <= 2)
        step_count++;
    }
    if (total_intervals > 0) {
      float raw_step_ratio = static_cast<float>(step_count) / total_intervals;
  ...
  if (subject.noteCount() >= 2) {
    ...
    float repeated_pitch_ratio =
        static_cast<float>(repeated) / static_cast<float>(subject.noteCount() - 1);
  ...
  if (subject.noteCount() >= 2) {
    ...
    float repeated_rhythm_ratio =
        static_cast<float>(same_dur) / static_cast<float>(subject.noteCount() - 1);
...
float SubjectValidator::scoreRhythmDiversity(const Subject& subject) const {
  ...
  float dominant_ratio = static_cast<float>(max_count) / static_cast<float>(subject.noteCount());

  if (dominant_ratio <= 0.5f)
    return 1.0f;
...
float SubjectValidator::scoreContourBalance(const Subject& subject) const {
  ...
  int leap_count = 0;
  for (size_t idx = 1; idx < subject.notes.size(); ++idx) {
    int curr_interval = directedInterval(subject.notes[idx - 1].pitch, subject.notes[idx].pitch);
    ...
    if (abs_curr >= 3) {
      ...
      if (idx + 1 < subject.notes.size()) {
        int next_interval =
            directedInterval(subject.notes[idx].pitch, subject.notes[idx + 1].pitch);
        bool opposite_dir =
            (curr_interval > 0 && next_interval < 0) || (curr_interval < 0 && next_interval > 0);
  ...
  if (leap_count > 0) {
    contrary_bonus =
        std::min(0.2f, static_cast<float>(contrary_motion_bonus_count) / leap_count * 0.3f);
  }
...
float SubjectValidator::scoreTonalStability(const Subject& subject) const {
  ...
  const int* scale =
      getScaleIntervals(subject.is_minor ? ScaleType::HarmonicMinor : ScaleType::Major);
  ...
  float scale_ratio =
      static_cast<float>(scale_tone_count) / static_cast<float>(subject.notes.size());
...
float SubjectValidator::scoreAnswerCompatibility(const Subject& subject) const {
  ...
  float chromatic_ratio =
      static_cast<float>(chromatic_count) / static_cast<float>(subject.noteCount());

  // No chromaticism = 1.0; heavy chromaticism (>30%) = 0.0.
  if (chromatic_ratio <= 0.05f)
    return 1.0f;
  if (chromatic_ratio >= 0.3f)
    return 0.0f;
