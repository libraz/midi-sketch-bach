...
float scoreIntervalPair(int subject_ivl, int toccata_ivl) {
  ...
  bool same_sign = (subject_ivl > 0 && toccata_ivl > 0) || (subject_ivl < 0 && toccata_ivl < 0) ||
...
float scoreSubsequence(const std::vector<int>& subject, size_t s_start,
                       const std::vector<int>& variant, size_t v_start, size_t len) {
  float total = 0.0f;
    ...
    for (size_t s_start = 0; s_start + sub_len <= subject_len; ++s_start) {
      ...
      for (const auto& variant : variants) {
        for (size_t v_start = 0; v_start + sub_len <= variant.size(); ++v_start) {
          float score = scoreSubsequence(subject_intervals, s_start, variant, v_start, sub_len);
          if (score > max_score)
            max_score = score;
        }
      }
