...
int normalizeEndingPitch(int target_pitch_class, int prev_pitch, int max_leap, Key key,
  ...
  if (prev_pitch > ceil) {
    ...
    if (std::abs(nearest_target + 12 - prev_pitch) < std::abs(nearest_target - prev_pitch)) {
      nearest_target += 12;
    }
...
std::vector<SkeletonSlot> buildRhythmSkeleton(const SubjectAnchors& a, const MotifTemplate& motif_a,
  ...
  for (size_t idx = 0; idx < motif_a.degree_offsets.size() && current_tick < a.climax_tick; ++idx) {
    Tick duration = (idx < motif_a.durations.size()) ? motif_a.durations[idx] : kTicksPerBeat;

    // Apply acceleration profile before pair substitution.
    // Progress: 0.0 at start, 1.0 at end of motif phase.
    float accel_progress =
        (a.total_ticks > 0) ? static_cast<float>(current_tick) / static_cast<float>(a.total_ticks)
                            : 0.0f;
    ...
    if (current_tick + duration > a.climax_tick) {
      ...
      if (duration < kTicksPerBeat / 4)
        break;
    }

    NoteFunction func =
        (idx < motif_a.functions.size()) ? motif_a.functions[idx] : NoteFunction::StructuralTone;
    slots.push_back({current_tick, duration, SlotPhase::kMotifA, motif_a.degree_offsets[idx],
                     motif_a_count, func});
  ...
  if (current_tick < a.total_ticks) {
    ...
    slots.push_back({current_tick, climax_dur, SlotPhase::kClimax, 0, 0, NoteFunction::ClimaxTone});
    current_tick += climax_dur;
  }

  // --- Motif B slots ---
  size_t motif_b_count = 0;
  for (size_t idx = 0; idx < motif_b.degree_offsets.size() && current_tick < a.total_ticks; ++idx) {
    Tick duration = (idx < motif_b.durations.size()) ? motif_b.durations[idx] : kTicksPerBeat;

    // Apply acceleration profile to Motif B as well.
    float accel_progress_b =
        (a.total_ticks > 0) ? static_cast<float>(current_tick) / static_cast<float>(a.total_ticks)
                            : 0.0f;
    if (accel_profile.curve_type == AccelCurveType::EaseIn) {
    ...
    NoteFunction func_b =
        (idx < motif_b.functions.size()) ? motif_b.functions[idx] : NoteFunction::StructuralTone;
    slots.push_back({current_tick, duration, SlotPhase::kMotifB, motif_b.degree_offsets[idx],
                     motif_b_count, func_b});
    current_tick += duration;
    ++motif_b_count;
  }
  ...
  // --- Cadence slots ---
  if (current_tick < a.total_ticks) {
    int tonic_abs_degree =
        scale_util::pitchToAbsoluteDegree(clampPitch(a.tonic_pitch, 0, 127), a.key, a.scale);
    ...
    for (int ci = start_idx; ci < cadence.count && current_tick < a.total_ticks; ++ci) {
      ...
      slots.push_back({current_tick, dur, SlotPhase::kCadence, degree_abs, cadence_count,
...
float evaluateNoteFunctionFit(int candidate_pitch, int prev_pitch, NoteFunction function, Key key,
  ...
  switch (function) {
    ...
    case NoteFunction::Resolution: {
      ...
      if (interval == -1 || interval == -2 || interval == 1)
        return per_note;
      return 0.0f;
    }
...
std::vector<NoteEvent> generateLegacyPitchPath(const SubjectAnchors& a,
                                               const std::vector<SkeletonSlot>& skeleton,
                                               const ArchetypePolicy& policy, uint32_t path_seed) {
  ...
  for (size_t si = 0; si < skeleton.size(); ++si) {
    ...
    switch (slot.phase) {
      case SlotPhase::kMotifA: {
        ...
        int target_pitch = degreeToPitch(target_degree, kBaseNote, a.key_offset, a.scale);

        // Scale toward climax: interpolate pitch between start and climax.
        float progress = (a.climax_tick > 0) ? static_cast<float>(slot.start_tick) /
                                                   static_cast<float>(a.climax_tick)
                                             : 0.0f;
        int interp_pitch =
            a.start_pitch +
            static_cast<int>(static_cast<float>(a.climax_pitch - a.start_pitch) * progress);
        ...
        if (slot.phase_index > 0 && rng::rollProbability(gen, a.fluctuation_rate)) {
        ...
        pitch = snapToScale(degreeToPitch(adjusted_degree, kBaseNote, a.key_offset, a.scale), a.key,
                            a.scale, a.pitch_floor, a.climax_pitch);
        ...
        if (slot.phase_index > 0 && slot.phase_index <= 2 && !result.empty()) {
          ...
          int head_interval =
              (result.size() >= 2)
                  ? std::abs(static_cast<int>(result[1].pitch) - static_cast<int>(result[0].pitch))
                  : 0;
          if (head_interval >= 5 && rng::rollProbability(gen, 0.70f)) {
            int step_dir = (result.back().pitch > result[0].pitch) ? -1 : 1;
            int step_pitch =
                snapToScale(prev_p + step_dir * 2, a.key, a.scale, a.pitch_floor, a.pitch_ceil);
            pitch = step_pitch;
          }
        }

        int prev_pitch = result.empty() ? -1 : static_cast<int>(result.back().pitch);
        pitch = avoidUnison(pitch, prev_pitch, a.key, a.scale, a.pitch_floor, a.climax_pitch);
        pitch = clampLeap(pitch, prev_pitch, a.character, a.key, a.scale, a.pitch_floor,
                          a.climax_pitch, gen, &large_leap_count);
      ...
      case SlotPhase::kClimax: {
        ...
        if (!result.empty()) {
          ...
          clamped_climax = clampLeap(a.climax_pitch, prev, a.character, a.key, a.scale,
                                     a.pitch_floor, a.pitch_ceil, gen, &large_leap_count);
        }
        pitch = clamped_climax;
        actual_climax_pitch = clamped_climax;
        actual_climax_abs_degree =
            scale_util::pitchToAbsoluteDegree(clampPitch(clamped_climax, 0, 127), a.key, a.scale);
      ...
      case SlotPhase::kMotifB: {
        ...
        float remaining_ratio = (a.total_ticks > a.climax_tick)
                                    ? static_cast<float>(slot.start_tick - a.climax_tick) /
                                          static_cast<float>(a.total_ticks - a.climax_tick)
                                    : 1.0f;
        int interp_pitch =
            actual_climax_pitch +
            static_cast<int>(static_cast<float>(a.tonic_pitch - actual_climax_pitch) *
                             remaining_ratio);

        int target_degree_abs = actual_climax_abs_degree + offset;
        int target_pitch =
            static_cast<int>(scale_util::absoluteDegreeToPitch(target_degree_abs, a.key, a.scale));
        ...
        pitch = snapToScale(static_cast<int>(scale_util::absoluteDegreeToPitch(adjusted_abs_degree,
                                                                               a.key, a.scale)),
                            a.key, a.scale, a.pitch_floor, actual_climax_pitch);

        int prev_pitch = result.empty() ? -1 : static_cast<int>(result.back().pitch);
        pitch = avoidUnison(pitch, prev_pitch, a.key, a.scale, a.pitch_floor, actual_climax_pitch);
        pitch = clampLeap(pitch, prev_pitch, a.character, a.key, a.scale, a.pitch_floor,
                          actual_climax_pitch, gen, &large_leap_count);
      ...
      case SlotPhase::kCadence: {
        pitch = snapToScale(
            static_cast<int>(scale_util::absoluteDegreeToPitch(slot.degree_hint, a.key, a.scale)),
            a.key, a.scale, a.pitch_floor, actual_climax_pitch);
    ...
    if (!result.empty()) {
      ...
      if (simple == 6) {
        ...
        if (prev_dur <= 240 && slot.duration <= 240)
          exempt = true;
        ...
        if (!exempt) {
          ...
          candidates[0].cand_pitch =
              snapToScale(pitch + (-direction), a.key, a.scale, a.pitch_floor, a.pitch_ceil);
          // (2) Same direction +-1.
          candidates[1].cand_pitch =
              snapToScale(pitch + direction, a.key, a.scale, a.pitch_floor, a.pitch_ceil);
          // (3) Same direction +2.
          candidates[2].cand_pitch =
              snapToScale(pitch + direction * 2, a.key, a.scale, a.pitch_floor, a.pitch_ceil);
  ...
  if (!result.empty()) {
    int prev_pitch_for_ending = (result.size() >= 2)
    ...
    if (rng::rollProbability(gen, dominant_rate)) {
      ...
      int ending = normalizeEndingPitch(dom_pc, prev_pitch_for_ending, max_leap, a.key, a.scale,
      ...
      int ending = normalizeEndingPitch(tonic_pc, prev_pitch_for_ending, max_leap, a.key, a.scale,
                                        a.pitch_floor, a.pitch_ceil);
      result.back().pitch = static_cast<uint8_t>(ending);
    }
  }
  ...
  for (size_t i = 1; i < result.size(); ++i) {
    ...
    if (std::abs(interval) > post_max_leap) {
      ...
      for (int attempt = post_max_leap; attempt >= 0; --attempt) {
        ...
        candidate = std::max(a.pitch_floor, std::min(a.pitch_ceil, candidate));
        int snapped = snapToScale(candidate, a.key, a.scale, a.pitch_floor, a.pitch_ceil);
  ...
  for (int tritone_pass = 0; tritone_pass < 2; ++tritone_pass) {
    ...
    for (size_t idx = 1; idx < result.size(); ++idx) {
      ...
      // Short note exemption: both notes <= 240 ticks.
      if (result[idx - 1].duration <= 240 && result[idx].duration <= 240)
        continue;
      ...
      for (int delta : {1, -1, 2, -2}) {
        ...
        int snapped = snapToScale(shifted, a.key, a.scale, a.pitch_floor, a.pitch_ceil);
        int new_simple = interval_util::compoundToSimple(std::abs(snapped - prev_p));
        if (new_simple == 6)
        ...
        if (idx + 1 < result.size()) {
          ...
          int fwd_simple = interval_util::compoundToSimple(std::abs(snapped - next_p));
          if (fwd_simple == 6)
            continue;
        }
...
std::vector<NoteEvent> generateKerngestaltPath(const SubjectAnchors& anchors,
                                               const std::vector<SkeletonSlot>& skeleton,
                                               const ArchetypePolicy& policy,
                                               const KerngestaltCell& cell, uint32_t path_seed,
  ...
  for (size_t idx = 0; idx < cell.intervals.size(); ++idx) {
    ...
    Tick cell_slot_tick = (cell_start_slot + idx + 1 < skeleton.size())
                              ? skeleton[cell_start_slot + idx + 1].start_tick
                              : 0;
    ...
    if (is_chromatic_cell && is_semitone && on_weak_beat) {
      ...
      cell_pitches[idx + 1] = std::max(anchors.pitch_floor, std::min(anchors.pitch_ceil, raw));
    } else {
      cell_pitches[idx + 1] =
          snapToScale(raw, anchors.key, anchors.scale, anchors.pitch_floor, anchors.pitch_ceil);
    }
  }
  ...
  if (octave_shift != 0) {
    for (auto& pch : cell_pitches)
      pch += octave_shift;
  }

  // Verify all cell pitches are in range after shift.
  for (int pch : cell_pitches) {
    if (pch < anchors.pitch_floor || pch > anchors.pitch_ceil)
      return {};
  }
    ...
    // --- Remaining slots: phase-based logic ---
      ...
      switch (slot.phase) {
        case SlotPhase::kMotifA: {
          ...
          float progress = (anchors.climax_tick > 0) ? static_cast<float>(slot.start_tick) /
                                                           static_cast<float>(anchors.climax_tick)
                                                     : 0.0f;
          int interp_pitch =
              anchors.start_pitch +
              static_cast<int>(static_cast<float>(anchors.climax_pitch - anchors.start_pitch) *
                               progress);
          ...
          if (slot.phase_index > 0 && rng::rollProbability(gen, anchors.fluctuation_rate)) {
            adjusted_degree += rng::rollProbability(gen, 0.5f) ? 1 : -1;
          }
          ...
          if (slot.phase_index > 0 && slot.phase_index <= 2 && !result.empty()) {
            int prev_p = static_cast<int>(result.back().pitch);
            int head_interval = (result.size() >= 2) ? std::abs(static_cast<int>(result[1].pitch) -
                                                                static_cast<int>(result[0].pitch))
                                                     : 0;
            if (head_interval >= 5 && rng::rollProbability(gen, 0.70f)) {
              int step_dir = (result.back().pitch > result[0].pitch) ? -1 : 1;
              int step_pitch = snapToScale(prev_p + step_dir * 2, anchors.key, anchors.scale,
                                           anchors.pitch_floor, anchors.pitch_ceil);
              pitch = step_pitch;
            }
          }
          ...
          int prev_pitch = result.empty() ? -1 : static_cast<int>(result.back().pitch);
          pitch = avoidUnison(pitch, prev_pitch, anchors.key, anchors.scale, anchors.pitch_floor,
                              anchors.climax_pitch);
          pitch = clampLeap(pitch, prev_pitch, anchors.character, anchors.key, anchors.scale,
                            anchors.pitch_floor, anchors.climax_pitch, gen, &large_leap_count);
        ...
        case SlotPhase::kClimax: {
          ...
          if (!result.empty()) {
            int prev = static_cast<int>(result.back().pitch);
            clamped_climax =
                clampLeap(anchors.climax_pitch, prev, anchors.character, anchors.key, anchors.scale,
                          anchors.pitch_floor, anchors.pitch_ceil, gen, &large_leap_count);
          }
          pitch = clamped_climax;
          actual_climax_pitch = clamped_climax;
          actual_climax_abs_degree = scale_util::pitchToAbsoluteDegree(
              clampPitch(clamped_climax, 0, 127), anchors.key, anchors.scale);
        ...
        case SlotPhase::kMotifB: {
          ...
          if (!cell_reappeared) {
            ...
            for (size_t fi = si; fi < skeleton.size(); ++fi) {
              if (skeleton[fi].phase == SlotPhase::kMotifB)
                ++remaining_motifb;
            }
            if (remaining_motifb >= cell_note_count) {
              int prev_p =
                  result.empty() ? anchors.start_pitch : static_cast<int>(result.back().pitch);
              ...
              for (int attempt = 0; attempt < 2; ++attempt) {
                ...
                for (size_t ri = 0; ri < cell.intervals.size(); ++ri) {
                  int interval = (attempt == 0) ? -cell.intervals[ri] : cell.intervals[ri];
                  int raw = trial_pitches[ri] + interval;
                  trial_pitches[ri + 1] = snapToScale(raw, anchors.key, anchors.scale,
                                                      anchors.pitch_floor, anchors.pitch_ceil);
          ...
          if (!(cell_reappeared && si >= reappearance_start && si <= reappearance_end &&
            ...
            if (cell_reappeared && si == reappearance_start) {
              ...
              int interp_pitch =
                  actual_climax_pitch +
                  static_cast<int>(static_cast<float>(anchors.tonic_pitch - actual_climax_pitch) *
                                   remaining_ratio);

              int target_degree_abs = actual_climax_abs_degree + offset;
              int target_pitch = static_cast<int>(
                  scale_util::absoluteDegreeToPitch(target_degree_abs, anchors.key, anchors.scale));
              ...
              pitch =
                  snapToScale(static_cast<int>(scale_util::absoluteDegreeToPitch(
              ...
              pitch = clampLeap(pitch, prev_pitch, anchors.character, anchors.key, anchors.scale,
                                anchors.pitch_floor, actual_climax_pitch, gen, &large_leap_count);
        ...
        case SlotPhase::kCadence: {
          pitch = snapToScale(static_cast<int>(scale_util::absoluteDegreeToPitch(
    ...
    if (!result.empty()) {
      ...
      if (simple == 6) {
        ...
        if (!exempt) {
          ...
          TritoneCandidate candidates[3];
          candidates[0].cand_pitch = snapToScale(pitch + (-direction), anchors.key, anchors.scale,
                                                 anchors.pitch_floor, anchors.pitch_ceil);
          candidates[1].cand_pitch = snapToScale(pitch + direction, anchors.key, anchors.scale,
                                                 anchors.pitch_floor, anchors.pitch_ceil);
          candidates[2].cand_pitch = snapToScale(pitch + direction * 2, anchors.key, anchors.scale,
                                                 anchors.pitch_floor, anchors.pitch_ceil);
  ...
  if (!result.empty()) {
    ...
    if (!last_in_cell) {
      ...
      float dominant_rate = policy.dominant_ending_prob;
      if (rng::rollProbability(gen, dominant_rate)) {
        int dom_pc = getPitchClass(
            static_cast<uint8_t>(degreeToPitch(4, kBaseNote, anchors.key_offset, anchors.scale)));
        int ending = normalizeEndingPitch(dom_pc, prev_pitch_for_ending, max_leap, anchors.key,
                                          anchors.scale, anchors.pitch_floor, anchors.pitch_ceil);
        result.back().pitch = static_cast<uint8_t>(ending);
      } else {
        int tonic_pc = getPitchClass(static_cast<uint8_t>(anchors.tonic_pitch));
        int ending = normalizeEndingPitch(tonic_pc, prev_pitch_for_ending, max_leap, anchors.key,
                                          anchors.scale, anchors.pitch_floor, anchors.pitch_ceil);
        result.back().pitch = static_cast<uint8_t>(ending);
      }
    }
  ...
  for (size_t idx = 1; idx < result.size(); ++idx) {
    ...
    if (idx >= out_cell_window.start_idx && idx <= out_cell_window.end_idx) {
      int prev_p = static_cast<int>(result[idx - 1].pitch);
      int cur_p = static_cast<int>(result[idx].pitch);
      if (std::abs(cur_p - prev_p) > post_max_leap)
        return {};
      continue;
    }
    ...
    if (std::abs(interval) > post_max_leap) {
      ...
      for (int attempt = post_max_leap; attempt >= 0; --attempt) {
        int candidate = prev_p + direction * attempt;
        candidate = std::max(anchors.pitch_floor, std::min(anchors.pitch_ceil, candidate));
        int snapped = snapToScale(candidate, anchors.key, anchors.scale, anchors.pitch_floor,
                                  anchors.pitch_ceil);
        if (std::abs(snapped - prev_p) <= post_max_leap) {
  ...
  for (int tritone_pass = 0; tritone_pass < 2; ++tritone_pass) {
    ...
    for (size_t idx = 1; idx < result.size(); ++idx) {
      ...
      for (int delta : {1, -1, 2, -2}) {
        int shifted = cur_p + delta;
        int snapped = snapToScale(shifted, anchors.key, anchors.scale, anchors.pitch_floor,
                                  anchors.pitch_ceil);
        int new_simple = interval_util::compoundToSimple(std::abs(snapped - prev_p));
        if (new_simple == 6)
          continue;
...
/// @brief Keep minor-mode leading tones as short directed tones, not long
/// structural supports.
///
/// A prolonged raised seventh in a minor subject makes the answer inherit a
/// prolonged raised seventh in the dominant. In the exposition this can sound
/// like unsupported chromatic harmony rather than fugal direction. Long
/// structural tones use natural minor; short leading tones remain available
/// for cadential pull.
void temperProlongedMinorLeadingTones(std::vector<NoteEvent>& notes, Key key) {
  const int tonic_pc = getPitchClass(static_cast<uint8_t>(key));
  const int raised_seventh_pc = (tonic_pc + 11) % 12;
  const int natural_seventh_pc = (tonic_pc + 10) % 12;

  for (auto& note : notes) {
    if (note.duration <= kTicksPerBeat)
      continue;
    if (getPitchClass(note.pitch) != raised_seventh_pc)
      continue;

    int lowered = static_cast<int>(note.pitch) - 1;
    if (getPitchClass(clampPitch(lowered, 0, 127)) != natural_seventh_pc) {
      continue;
    }
    note.pitch = static_cast<uint8_t>(clampPitch(lowered, 0, 127));
  }
}

struct RestlessMinorSubjectTemplate {
  std::vector<NoteEvent> notes;
  CellWindow cell_window;
};

RestlessMinorSubjectTemplate buildRestlessMinorBaroqueSubject(Key key, uint32_t seed) {
  const int transpose = static_cast<int>(key) - static_cast<int>(Key::G);
  struct PatternNote {
    int pitch;
    Tick duration;
  ...
  static const PatternNote kPattern0[] = {
      {55, kTicksPerBeat, BachNoteSource::SubjectCore},
      {57, kTicksPerBeat / 2, BachNoteSource::SubjectCore},
      {58, kTicksPerBeat / 2, BachNoteSource::SubjectCore},
      {55, kTicksPerBeat, BachNoteSource::SubjectCore},
      {62, kTicksPerBeat / 2, BachNoteSource::SubjectCore},
      {60, kTicksPerBeat / 2, BachNoteSource::FugueSubject},
      {58, kTicksPerBeat / 2, BachNoteSource::FugueSubject},
      {57, kTicksPerBeat / 2, BachNoteSource::FugueSubject},
      {54, kTicksPerBeat / 2, BachNoteSource::FugueSubject},
      {55, kTicksPerBeat / 2, BachNoteSource::FugueSubject},
      {50, kTicksPerBeat + kTicksPerBeat / 2, BachNoteSource::FugueSubject},
  };
  static const PatternNote kPattern1[] = {
      {55, kTicksPerBeat, BachNoteSource::SubjectCore},
      {58, kTicksPerBeat / 2, BachNoteSource::SubjectCore},
      {60, kTicksPerBeat / 2, BachNoteSource::SubjectCore},
      {57, kTicksPerBeat, BachNoteSource::SubjectCore},
      {62, kTicksPerBeat / 2, BachNoteSource::SubjectCore},
      {60, kTicksPerBeat / 2, BachNoteSource::FugueSubject},
      {58, kTicksPerBeat / 2, BachNoteSource::FugueSubject},
      {57, kTicksPerBeat / 2, BachNoteSource::FugueSubject},
  ...
  static const PatternNote kPattern2[] = {
      {55, kTicksPerBeat, BachNoteSource::SubjectCore},
      {57, kTicksPerBeat / 2, BachNoteSource::SubjectCore},
      {60, kTicksPerBeat / 2, BachNoteSource::SubjectCore},
      {58, kTicksPerBeat, BachNoteSource::SubjectCore},
      {62, kTicksPerBeat / 2, BachNoteSource::SubjectCore},
      {60, kTicksPerBeat / 2, BachNoteSource::FugueSubject},
      {58, kTicksPerBeat / 2, BachNoteSource::FugueSubject},
      {57, kTicksPerBeat / 2, BachNoteSource::FugueSubject},
  ...
  const PatternNote* pattern = kPattern0;
  size_t pattern_size = std::size(kPattern0);
  switch (seed % 3u) {
    case 1u:
      pattern = kPattern1;
      pattern_size = std::size(kPattern1);
      break;
    case 2u:
      pattern = kPattern2;
      pattern_size = std::size(kPattern2);
      break;
    default:
      break;
  }

  RestlessMinorSubjectTemplate result;
  Tick tick = 0;
  for (size_t idx = 0; idx < pattern_size; ++idx) {
    const auto& spec = pattern[idx];
    NoteEvent note;
    note.start_tick = tick;
    note.duration = spec.duration;
    note.pitch = clampPitch(spec.pitch + transpose, 0, 127);
    note.velocity = 80;
    note.voice = 0;
    note.source = spec.source;
    result.notes.push_back(note);
    tick += spec.duration;
  }
  result.cell_window = {0, 4, true};
  return result;
}
...
Subject SubjectGenerator::generate(const FugueConfig& config, uint32_t seed) const {
  Subject subject;
  ...
  if (bars < policy.min_subject_bars)
    bars = policy.min_subject_bars;
  if (bars > policy.max_subject_bars)
    bars = policy.max_subject_bars;
  if (bars < 1)
    bars = 1;
  if (bars > 4)
    bars = 4;
  ...
  switch (config.character) {
    case SubjectCharacter::Severe:
      anacrusis_prob = rng::rollFloat(anacrusis_gen, 0.20f, 0.40f);
      break;
    case SubjectCharacter::Playful:
      anacrusis_prob = rng::rollFloat(anacrusis_gen, 0.60f, 0.80f);
      break;
    case SubjectCharacter::Noble:
      anacrusis_prob = rng::rollFloat(anacrusis_gen, 0.30f, 0.50f);
      break;
    case SubjectCharacter::Restless:
      anacrusis_prob = rng::rollFloat(anacrusis_gen, 0.50f, 0.70f);
      break;
  }
  // Clamp anacrusis probability to archetype bounds.
  anacrusis_prob = std::clamp(anacrusis_prob, policy.min_anacrusis_prob, policy.max_anacrusis_prob);
  bool has_anacrusis = rng::rollProbability(anacrusis_gen, anacrusis_prob);
  if (config.is_minor && config.character == SubjectCharacter::Restless) {
    has_anacrusis = false;
  }
  ...
  if (has_anacrusis) {
    if (config.character == SubjectCharacter::Playful ||
      ...
      anacrusis_ticks =
          rng::rollProbability(anacrusis_gen, 0.5f) ? kTicksPerBeat : kTicksPerBeat * 2;
    } else {
  ...
  if (config.is_minor && config.character == SubjectCharacter::Restless && bars == 2 &&
      config.toccata_core_intervals.empty()) {
    auto baroque_template = buildRestlessMinorBaroqueSubject(config.key, seed);
    best_result.notes = std::move(baroque_template.notes);
    best_result.cell_window = baroque_template.cell_window;
  } else {
    for (int retry = 0; retry < kMaxRetries; ++retry) {
      auto result = generateNotes(config, bars, seed + static_cast<uint32_t>(retry));
      Subject candidate;
      ...
      if (config.toccata_core_intervals.empty()) {
        ...
        comp =
            (base_comp * w + arch_comp * (1.0f - w)) * (1.0f - kToccataAlpha) + toc * kToccataAlpha;
      }
  ...
  if (config.is_minor) {
    temperProlongedMinorLeadingTones(subject.notes, config.key);
  }
...
SubjectGenerator::GenerateResult SubjectGenerator::generateNotes(const FugueConfig& config,
                                                                 uint8_t bars,
                                                                 uint32_t seed) const {
  ...
  switch (character) {
    case SubjectCharacter::Severe:
      tonic_prob = rng::rollFloat(anchor_rng, 0.55f, 0.70f);
      break;
    case SubjectCharacter::Playful:
      tonic_prob = rng::rollFloat(anchor_rng, 0.40f, 0.55f);
      break;
    case SubjectCharacter::Noble:
      tonic_prob = rng::rollFloat(anchor_rng, 0.60f, 0.70f);
      break;
    case SubjectCharacter::Restless:
      tonic_prob = rng::rollFloat(anchor_rng, 0.45f, 0.60f);
      break;
  }
  ...
  int tonic_abs = scale_util::pitchToAbsoluteDegree(static_cast<uint8_t>(tonic_base), key, scale);
  int start_abs = scale_util::pitchToAbsoluteDegree(static_cast<uint8_t>(start_pitch), key, scale);
  int floor_abs = std::min(start_abs, tonic_abs);
  int ceil_abs = floor_abs + params.max_range_degrees;

  int pitch_floor = static_cast<int>(scale_util::absoluteDegreeToPitch(floor_abs, key, scale));
  int pitch_ceil = static_cast<int>(scale_util::absoluteDegreeToPitch(ceil_abs, key, scale));
  ...
  int climax_abs =
      floor_abs + static_cast<int>(static_cast<float>(ceil_abs - floor_abs) * goal.pitch_ratio);
  if (climax_abs < start_abs + 3) {
    climax_abs = std::min(start_abs + 3, ceil_abs);
  }
  int climax_pitch = static_cast<int>(scale_util::absoluteDegreeToPitch(climax_abs, key, scale));
  climax_pitch = std::max(pitch_floor, std::min(pitch_ceil, climax_pitch));

  Tick raw_climax_tick = static_cast<Tick>(static_cast<float>(total_ticks) * goal.position_ratio);
  ...
  switch (character) {
    case SubjectCharacter::Severe:
      fluctuation_rate = rng::rollFloat(anchor_rng, 0.10f, 0.25f);
      break;
    case SubjectCharacter::Playful:
      fluctuation_rate = rng::rollFloat(anchor_rng, 0.20f, 0.35f);
      break;
    case SubjectCharacter::Noble:
      fluctuation_rate = rng::rollFloat(anchor_rng, 0.10f, 0.20f);
      break;
    case SubjectCharacter::Restless:
      fluctuation_rate = rng::rollFloat(anchor_rng, 0.25f, 0.40f);
      break;
  }
  ...
  int climax_abs_degree =
      scale_util::pitchToAbsoluteDegree(clampPitch(climax_pitch, 0, 127), key, scale);

  CadentialFormula cadence = getCadentialFormula(character);

  SubjectAnchors anchors{start_degree, start_pitch, tonic_pitch, climax_pitch,      climax_tick,
                         total_ticks,  pitch_floor, pitch_ceil,  climax_abs_degree, key,
                         key_offset,   scale,       character,   fluctuation_rate};
  ...
  KerngestaltType kern_type = selectKerngestaltType(character, config.archetype, type_rng);
  ...
  int num_candidates =
      (policy.path_candidates > 0) ? policy.path_candidates : kDefaultPathCandidates;
  ...
  for (int idx = 0; idx < num_candidates; ++idx) {
    ...
    CellWindow cw;
    auto candidate = generateKerngestaltPath(anchors, skeleton, policy, cell, path_seed, cw);
    ...
    if (!archetype_scorer.checkHardGate(sub, policy))
      continue;
    ...
    for (size_t ni = 0; ni < eval_count; ++ni) {
      ...
      note_func_bonus +=
          evaluateNoteFunctionFit(static_cast<int>(candidate[ni].pitch), prev_p,
    ...
    if (config.toccata_core_intervals.empty()) {
      ...
      float toc = computeToccataAffinity(subj_ivls, config.toccata_core_intervals);
      comp = base_comp * 0.60f + arch_comp * 0.25f + note_func_bonus * 0.10f + toc * 0.05f;
    }
  ...
  if (best.tier == 2 || best.notes.empty()) {
    for (int idx = 0; idx < num_candidates; ++idx) {
      uint32_t path_seed = seed ^ (static_cast<uint32_t>(idx) * 0x7A3B9C1Du) ^ arch_mix;
      auto candidate = generateLegacyPitchPath(anchors, skeleton, policy, path_seed);
