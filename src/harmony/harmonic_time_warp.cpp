...
float computeBeatWeight(int grid_bar, int beat_in_bar, const GoldbergStructuralGrid& grid,
                        MeterProfile meter) {
...
std::vector<float> computePhraseDeltas(int phrase_idx, int beats_per_bar,
                                       const GoldbergStructuralGrid& grid, MeterProfile meter,
...
void applyHarmonicTimeWarp(std::vector<NoteEvent>& notes, const GoldbergStructuralGrid& grid,
  ...
  if (beat_ticks == 0 || phrase_ticks == 0)
    return;
  ...
  for (int p = 0; p < total_phrases; ++p) {
    ...
    all_deltas[p] =
        computePhraseDeltas(p, beats_per_bar, grid, desc.meter_profile, max_delta, asymmetry);
  }
  ...
  for (auto& note : notes) {
    ...
    if (beat_in_phrase >= beats_per_phrase)
      beat_in_phrase = beats_per_phrase - 1;
    ...
    int64_t new_tick =
        static_cast<int64_t>(note.start_tick) + static_cast<int64_t>(std::round(displacement));
    ...
    Tick new_dur = static_cast<Tick>(
        std::max(1.0f, std::round(static_cast<float>(note.duration) * dur_scale)));
