...
VoiceCache buildVoiceCache(const std::vector<NoteEvent>& notes, uint8_t num_voices) {
  ...
  for (uint8_t idx = 0; idx < num_voices; ++idx)
    voices[idx] = voiceNotes(notes, idx);
  return voices;
}

/// @brief Emit a FailIssue for a detected violation.
FailIssue makeIssue(FailSeverity sev, Tick tick, VoiceId va, VoiceId vb, const std::string& rule,
...
uint32_t countStructuralParallels(const VoiceCache& voices, uint8_t num_voices, Tick end) {
  uint32_t count = 0;
  for (uint8_t va_idx = 0; va_idx < num_voices; ++va_idx) {
    for (uint8_t vb_idx = va_idx + 1; vb_idx < num_voices; ++vb_idx) {
      int prev_a = -1, prev_b = -1;
  ...
  for (Tick beat = 0; beat + kTicksPerBeat < end; beat += kTicksPerBeat) {
    ...
    for (uint8_t va = 0; va < num_voices; ++va) {
      for (uint8_t vb = va + 1; vb < num_voices; ++vb) {
        ...
        if (pa < 0 || pb < 0)
          continue;
        if (classifyInterval(std::abs(pa - pb)) != IntervalQuality::Dissonance)
          continue;
        ++total;
...
CounterpointAnalysisResult analyzeCounterpoint(const std::vector<NoteEvent>& notes,
  ...
  if (num_voices >= 2 && result.parallel_perfect_count > 0) {
    auto voices = buildVoiceCache(notes, num_voices);
    Tick end = totalEndTick(notes);
    result.structural_parallel_count = countStructuralParallels(voices, num_voices, end);
  }
  ...
  if (beats > 0)
    result.overall_compliance_rate =
        clamp01(1.0f - static_cast<float>(violations) / static_cast<float>(beats));
  return result;
}
      ...
      for (Tick beat = 0; beat < end; beat += kTicksPerBeat) {
        ...
        if (interval_util::isPerfectConsonance(ci) && ps != cs && sd && std::abs(ma) > 2) {
          report.addIssue(makeIssue(FailSeverity::Warning, beat, va, vb, "hidden_perfect",
                                    "Hidden perfect interval between voices " + std::to_string(va) +
                                        " and " + std::to_string(vb)));
        }
        prev_a = ca;
        prev_b = cb;
      }
      ...
      for (uint8_t vb = va + 1; vb < num_voices; ++vb) {
        ...
        if (hi >= 0 && lo >= 0 && hi < lo) {
          ...
          report.addIssue(makeIssue(FailSeverity::Critical, beat, va, vb, "voice_crossing",
                                    "Voice " + std::to_string(va) + " (pitch " +
                                        std::to_string(hi) + ") crosses below voice " +
                                        std::to_string(vb) + " (pitch " + std::to_string(lo) +
                                        ")"));
        }
      }
    }
  ...
  for (uint8_t vid = 0; vid < num_voices; ++vid) {
    for (size_t idx = 1; idx < voices[vid].size(); ++idx) {
      if (absoluteInterval(voices[vid][idx].pitch, voices[vid][idx - 1].pitch) ==
          interval::kTritone) {
        report.addIssue(makeIssue(FailSeverity::Critical, voices[vid][idx].start_tick, vid, vid,
                                  "augmented_leap",
...
float leapResolutionRate(const std::vector<NoteEvent>& notes, uint8_t num_voices) {
  ...
  for (uint8_t vid = 0; vid < num_voices; ++vid) {
    ...
    for (size_t idx = 1; idx < sorted.size(); ++idx) {
      int leap = static_cast<int>(sorted[idx].pitch) - static_cast<int>(sorted[idx - 1].pitch);
      ...
      if (idx + 1 < sorted.size()) {
        int resolution =
            static_cast<int>(sorted[idx + 1].pitch) - static_cast<int>(sorted[idx].pitch);
...
float rhythmDiversityScore(const std::vector<NoteEvent>& notes, uint8_t num_voices) {
  ...
  for (const auto& dcount : duration_counts) {
    if (dcount.second > max_count)
      max_count = dcount.second;
  }
...
float textureDensityVariance(const std::vector<NoteEvent>& notes, uint8_t num_voices) {
  ...
  Tick max_tick = 0;
  for (const auto& note : notes) {
    if (note.start_tick < min_tick)
      min_tick = note.start_tick;
    Tick end = note.start_tick + note.duration;
    if (end > max_tick)
      max_tick = end;
  }
    ...
    for (uint8_t vb = va + 1; vb < num_voices; ++vb) {
      ...
      for (const auto& na : va_notes) {
        ...
        for (const auto& nb : vb_notes) {
          ...
          Tick tick_dist = (na.start_tick > nb.start_tick) ? na.start_tick - nb.start_tick
                                                           : nb.start_tick - na.start_tick;
          if (tick_dist > proximity_threshold)
            continue;
