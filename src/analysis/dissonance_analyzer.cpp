...
bool isPassingTone(const std::vector<NoteEvent>& sorted_voice, size_t note_idx) {
  if (note_idx == 0 || note_idx + 1 >= sorted_voice.size())
    return false;
  ...
  if (std::abs(approach) < 1 || std::abs(approach) > 2)
    return false;
  if (std::abs(departure) < 1 || std::abs(departure) > 2)
    return false;
...
bool isAppoggiatura(const std::vector<NoteEvent>& sorted_voice, size_t note_idx,
  ...
  if (note_idx + 1 >= sorted_voice.size())
    return false;
  if (!isStrongBeatInBar(sorted_voice[note_idx].start_tick))
    return false;
...
bool isAnticipation(uint8_t pitch, Tick tick, Tick duration, const HarmonicTimeline& timeline) {
  ...
  if (next_event.tick == curr_event.tick)
    return false;
...
size_t findNoteIndex(const std::vector<NoteEvent>& sorted_voice, uint8_t pitch, Tick tick) {
...
DissonanceEvent makeEvent(DissonanceType type, DissonanceSeverity severity, Tick tick,
                          uint8_t pitch, uint8_t other_pitch, VoiceId voice_a, VoiceId voice_b,
                          int ivl, const std::string& desc) {
  DissonanceEvent ev;
  ev.type = type;
...
DissonanceAnalysisSummary buildSummary(const std::vector<DissonanceEvent>& events,
  ...
  for (const auto& ev : events) {
    ...
    switch (ev.type) {
      case DissonanceType::SimultaneousClash:
        ++s.simultaneous_clash_count;
        break;
      case DissonanceType::NonChordTone:
        ++s.non_chord_tone_count;
        break;
      case DissonanceType::SustainedOverChordChange:
        ++s.sustained_over_chord_change_count;
        break;
      case DissonanceType::NonDiatonicNote:
        ++s.non_diatonic_note_count;
        break;
    }
  }
  uint32_t total_beats =
      (total_duration > 0) ? static_cast<uint32_t>(total_duration / kTicksPerBeat) : 1;
  s.density_per_beat = static_cast<float>(s.total) / static_cast<float>(total_beats);
  float weighted = static_cast<float>(s.high_count) + static_cast<float>(s.medium_count) * 0.5f;
  ...
  for (Tick tick = 0; tick < end_tick; tick += kTicksPerBeat) {
    ...
    for (VoiceId va = 0; va < num_voices; ++va) {
      ...
      for (VoiceId vb = va + 1; vb < num_voices; ++vb) {
        ...
        int ivl =
            absoluteInterval(static_cast<uint8_t>(pitches[va]), static_cast<uint8_t>(pitches[vb]));
        ...
        if (quality == IntervalQuality::Dissonance &&
            interval_util::compoundToSimple(ivl) == interval::kPerfect4th && va != bass_voice &&
            vb != bass_voice) {
          continue;
        }
        ...
        if (quality == IntervalQuality::Dissonance) {
          ...
          if (severity != DissonanceSeverity::Low) {
            if (hasStructuralNoteAt(voices[va], tick) || hasStructuralNoteAt(voices[vb], tick)) {
              severity = DissonanceSeverity::Low;
            }
          }

          std::string desc =
              std::string(intervalToName(ivl)) + " clash - voice " + std::to_string(va) + " (" +
              pitchToNoteName(static_cast<uint8_t>(pitches[va])) + ") vs voice " +
              std::to_string(vb) + " (" + pitchToNoteName(static_cast<uint8_t>(pitches[vb])) + ")";

          results.push_back(makeEvent(DissonanceType::SimultaneousClash, severity, tick,
                                      static_cast<uint8_t>(pitches[va]),
                                      static_cast<uint8_t>(pitches[vb]), va, vb, ivl, desc));
        }
      }
    }
  }
...
std::vector<DissonanceEvent> detectNonChordTones(const std::vector<NoteEvent>& notes,
                                                 const HarmonicTimeline& timeline,
                                                 const HarmonicTimeline* generation_timeline) {
  std::vector<DissonanceEvent> results;
  if (notes.empty() || timeline.size() == 0)
    return results;
  ...
  for (const auto& note : notes) {
    ...
    if (!isChordTone(note.pitch, event)) {
      ...
      if (note.voice < voices.size()) {
        ...
        if (idx != SIZE_MAX) {
          if (isPassingTone(voices[note.voice], idx) || isNeighborTone(voices[note.voice], idx) ||
      ...
      results.push_back(makeEvent(DissonanceType::NonChordTone, severity, note.start_tick,
...
std::vector<DissonanceEvent> detectSustainedOverChordChange(const std::vector<NoteEvent>& notes,
                                                            ...
                                                            const HarmonicTimeline& timeline) {
  std::vector<DissonanceEvent> results;
  if (notes.empty() || timeline.size() < 2 || num_voices == 0)
    return results;
    ...
    for (const auto& note : notes) {
      ...
      if (note.start_tick < boundary_tick && note_end > boundary_tick) {
        if (!isChordTone(note.pitch, new_event)) {
          ...
          Tick preparation_span = boundary_tick - note.start_tick;
          if (preparation_span <= duration::kEighthNote) {
            severity = DissonanceSeverity::Low;
          }
          ...
          if (isChordTone(note.pitch, prev_event) && note.voice < voices.size()) {
            ...
            for (size_t ni = 0; ni < voice_notes.size(); ++ni) {
              if (voice_notes[ni].start_tick == note.start_tick &&
                ...
                if (ni + 1 < voice_notes.size()) {
                  int step =
                      static_cast<int>(note.pitch) - static_cast<int>(voice_notes[ni + 1].pitch);
                  // Resolution by descending step (1-2 semitones) to chord tone.
                  if (step >= 1 && step <= 2 && isChordTone(voice_notes[ni + 1].pitch, new_event)) {
                    severity = DissonanceSeverity::Low;
                  }
                }
                break;
          ...
          std::string desc =
              std::string(dissonanceTypeToString(DissonanceType::SustainedOverChordChange)) +
              " - voice " + std::to_string(note.voice) + " (" + pitchToNoteName(note.pitch) +
              ") sustained into " + chordDegreeToString(new_event.chord.degree) + " chord";

          results.push_back(makeEvent(DissonanceType::SustainedOverChordChange, severity,
  ...
  for (const auto& note : notes) {
    if (!isDiatonicInKey(note.pitch, key_sig.tonic, key_sig.is_minor)) {
      ...
      if (note.voice < voices.size()) {
        ...
        if (idx != SIZE_MAX) {
          is_ornamental =
              isPassingTone(voices[note.voice], idx) || isNeighborTone(voices[note.voice], idx);
        }
      }

      if (is_weak && is_ornamental) {
        severity = DissonanceSeverity::Low;
      } else if (getProtectionLevel(note.source) == ProtectionLevel::Immutable) {
        severity = DissonanceSeverity::Low;
      } else if (isStrongBeatInBar(note.start_tick)) {
        severity = DissonanceSeverity::High;
      }

      std::string desc = std::string(dissonanceTypeToString(DissonanceType::NonDiatonicNote)) +
                         " - voice " + std::to_string(note.voice) + " (" +
                         pitchToNoteName(note.pitch) + ") not diatonic in " +
                         keySignatureToString(key_sig);

      results.push_back(makeEvent(DissonanceType::NonDiatonicNote, severity, note.start_tick,
                                  note.pitch, 0, note.voice, 0, 0, desc));
    }
  }
...
std::vector<DissonanceEvent> detectNonDiatonicNotes(const std::vector<NoteEvent>& notes,
  ...
  for (const auto& note : notes) {
    ...
    if (!isDiatonicInKey(note.pitch, local_key.tonic, local_key.is_minor)) {
      ...
      if (is_weak && is_ornamental) {
        severity = DissonanceSeverity::Low;
      } else if (isChordTone(note.pitch, event)) {
        severity = DissonanceSeverity::Low;
      } else if (getProtectionLevel(note.source) == ProtectionLevel::Immutable) {
        severity = DissonanceSeverity::Low;
      } else if (isStrongBeatInBar(note.start_tick)) {
...
DissonanceAnalysisResult analyzeOrganDissonance(const std::vector<NoteEvent>& notes,
                                                uint8_t num_voices,
                                                const HarmonicTimeline& timeline,
                                                const KeySignature& key_sig,
                                                const HarmonicTimeline* generation_timeline) {
  ...
  auto phase4 = generation_timeline ? detectNonDiatonicNotes(notes, *generation_timeline)
  ...
  std::sort(result.events.begin(), result.events.end(),
            [](const DissonanceEvent& a, const DissonanceEvent& b) { return a.tick < b.tick; });
...
std::string DissonanceAnalysisResult::toTextSummary(const char* system_name,
                                                    uint8_t num_voices) const {
  ...
  oss << "\n";
  oss << "Total: " << summary.total << "  (High: " << summary.high_count
  ...
  for (const auto& ev : events) {
    if (ev.severity == DissonanceSeverity::High) {
      ...
      oss << "  Bar " << ev.bar << ", Beat " << static_cast<int>(ev.beat) << ": " << ev.description
          << "\n";
    }
  }
