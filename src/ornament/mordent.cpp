// Mordent ornament generation implementation.

#include "ornament/mordent.h"

namespace bach {

std::vector<NoteEvent> generateMordent(const NoteEvent& note, uint8_t lower_pitch) {
  std::vector<NoteEvent> result;

  // Need at least 4 ticks to split into 3 meaningful sub-notes (50/25/25).
  if (note.duration < 4) {
    result.push_back(note);
    return result;
  }

  result.reserve(3);

  const Tick half_dur = note.duration / 2;
  const Tick quarter_dur = note.duration / 4;
  const Tick last_dur = note.duration - half_dur - quarter_dur;  // Absorb rounding remainder.

  // Note 1: main pitch, 50% duration.
  NoteEvent first;
  first.start_tick = note.start_tick;
  first.duration = half_dur;
  first.pitch = note.pitch;
  first.velocity = note.velocity;
  first.voice = note.voice;
  result.push_back(first);

  // Note 2: lower neighbor, 25% duration.
  NoteEvent second;
  second.start_tick = note.start_tick + half_dur;
  second.duration = quarter_dur;
  second.pitch = lower_pitch;
  second.velocity = note.velocity;
  second.voice = note.voice;
  result.push_back(second);

  // Note 3: main pitch, 25% duration (+ rounding remainder).
  NoteEvent third;
  third.start_tick = note.start_tick + half_dur + quarter_dur;
  third.duration = last_dur;
  third.pitch = note.pitch;
  third.velocity = note.velocity;
  third.voice = note.voice;
  result.push_back(third);

  return result;
}

}  // namespace bach
