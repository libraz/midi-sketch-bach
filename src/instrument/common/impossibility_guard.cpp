...
bool isBassoSource(BachNoteSource source) {
  return source == BachNoteSource::GroundBass || source == BachNoteSource::ChaconneBass ||
         source == BachNoteSource::GoldbergBass || source == BachNoteSource::PedalPoint;
}
...
bool isOctaveShiftAcceptable(uint8_t original, uint8_t shifted, uint8_t prev_pitch) {
  if (prev_pitch == 0)
    return true;  // No previous note, accept any shift.
  int orig_dist = std::abs(static_cast<int>(original) - static_cast<int>(prev_pitch));
  int shift_dist = std::abs(static_cast<int>(shifted) - static_cast<int>(prev_pitch));
  if (shift_dist > orig_dist)
    return false;  // Interval increased.
  ...
  if (orig_dir > 0 && shift_dir < 0)
    return false;  // Direction reversed.
  if (orig_dir < 0 && shift_dir > 0)
    return false;
  return true;
}
...
std::function<uint8_t(uint8_t, ProtectionLevel, uint8_t)> makeFixPitchRange(uint8_t range_low,
                                                                            uint8_t range_high) {
  return
      [range_low, range_high](uint8_t pitch, ProtectionLevel level, uint8_t prev_pitch) -> uint8_t {
        // Already in range?
        ...
        if (up >= range_low && up <= range_high && up <= 127) {
          if (isOctaveShiftAcceptable(pitch, static_cast<uint8_t>(up), prev_pitch)) {
        ...
        if (down >= range_low && down <= range_high && down >= 0) {
          if (isOctaveShiftAcceptable(pitch, static_cast<uint8_t>(down), prev_pitch)) {
...
ImpossibilityGuard createBowedGuard() {
  ...
  guard.isPitchPlayable = [model](uint8_t pitch) { return model->isPitchPlayable(pitch); };
  ...
  guard.checkSounding = [model](const SoundingGroup& group) -> Violation {
    ...
    if (!model->isDoubleStopFeasible(group.notes[0]->pitch, group.notes[1]->pitch)) {
  ...
  guard.repairSounding = [](SoundingGroup& group) {
    ...
    std::sort(group.notes.begin(), group.notes.end(), [](const NoteEvent* a, const NoteEvent* b) {
    ...
    if (level_a == ProtectionLevel::Immutable && level_b == ProtectionLevel::Immutable) {
      std::fprintf(stderr,
...
ImpossibilityGuard createKeyboardGuard() {
  ...
  guard.isPitchPlayable = [model](uint8_t pitch) { return model->isPitchInRange(pitch); };
  ...
  guard.repairSounding = [model](SoundingGroup& group) {
    ...
    std::sort(group.notes.begin(), group.notes.end(),
              [](const NoteEvent* a, const NoteEvent* b) { return a->pitch < b->pitch; });
...
ImpossibilityGuard createOrganGuard() {
  ...
  ImpossibilityGuard guard;
  guard.isPitchPlayable = [low, high](uint8_t pitch) { return pitch >= low && pitch <= high; };
  guard.fixPitchRange = makeFixPitchRange(low, high);
...
uint32_t enforceImpossibilityGuard(std::vector<Track>& tracks, const ImpossibilityGuard& guard) {
  ...
  for (auto& track : tracks) {
    for (auto& note : track.notes) {
      if (note.duration == 0)
        continue;  // Skip already-dropped notes.
      ...
      auto level = getProtectionLevel(note.source);
      uint8_t prev = prev_pitch_map.count(note.voice) ? prev_pitch_map[note.voice] : 0;
  ...
  while (idx < events.size()) {
    ...
    for (int iteration = 0; iteration < 2; ++iteration) {
      ...
      group.notes.erase(std::remove_if(group.notes.begin(), group.notes.end(),
