...
inline bool isStructuralSource(BachNoteSource source) {
  return source == BachNoteSource::SubjectCore || source == BachNoteSource::FugueSubject ||
         ...
         source == BachNoteSource::Countersubject || source == BachNoteSource::FalseEntry ||
         source == BachNoteSource::Coda || source == BachNoteSource::SequenceNote ||
         source == BachNoteSource::CanonDux || source == BachNoteSource::CanonComes ||
         source == BachNoteSource::GoldbergAria || source == BachNoteSource::ToccataFigure ||
...
inline bool isPedalVoice(uint8_t voice, uint8_t num_voices, bool has_true_pedal = false) {
  if (num_voices < 3)
    return false;
  if (voice != num_voices - 1)
    return false;
  if (num_voices == 3)
    return has_true_pedal;
