...
BachCreateNoteResult createBachNote(CounterpointState* state, IRuleEvaluator* rules,
  ...
  if (state && rules && resolver) {
    PlacementResult placement =
        resolver->findSafePitch(*state, *rules, opts.voice, opts.desired_pitch, opts.tick,
                                opts.duration, opts.source, opts.next_pitch);
...
MelodicContext buildMelodicContextFromState(const CounterpointState& state, VoiceId voice_id) {
  ...
  if (ctx.prev_count >= 2 && ctx.prev_direction != 0) {
    ...
    for (int idx = static_cast<int>(voice_notes.size()) - 1; idx >= 1; --idx) {
      int diff =
          static_cast<int>(voice_notes[idx].pitch) - static_cast<int>(voice_notes[idx - 1].pitch);
...
std::vector<NoteEvent> postValidateNotes(
    ...
    const ProtectionOverrides& protection_overrides, bool stylus_phantasticus) {
  auto range_fn = [&voice_ranges](uint8_t voice, Tick /*tick*/) -> std::pair<uint8_t, uint8_t> {
    if (voice < voice_ranges.size())
      return voice_ranges[voice];
    return {0, 127};
  };
...
std::vector<NoteEvent> postValidateNotes(
  ...
  for (const auto& note : raw_notes) {
    ...
    ProtectionLevel prot = (note.voice < num_voices && voice_prot_override[note.voice] >= 0)
    ...
    if (prev) {
      ...
      if (voice_notes.size() >= 2) {
        ...
        opts.prev_direction = (note.pitch > prev->pitch) ? 1 : (note.pitch < prev->pitch) ? -1 : 0;
      }
    }
  ...
  //     leaps as part of the improvisatory rhetorical style
  {
    ...
    for (size_t idx = 0; idx < result.size(); ++idx) {
      ...
      if (vid < 5)
        voice_result_indices[vid].push_back(idx);
    }
    ...
    for (uint8_t vid = 0; vid < std::min(num_voices, static_cast<uint8_t>(5)); ++vid) {
      ...
      int max_leap =
          stylus_phantasticus ? std::max(base_max_leap, kPhantasticusMaxLeap) : base_max_leap;
      ...
      for (size_t pos = 1; pos < indices.size(); ++pos) {
        ...
        ProtectionLevel curr_prot =
            (curr.voice < num_voices && voice_prot_override[curr.voice] >= 0)
        ...
        int leap = std::abs(static_cast<int>(curr.pitch) - static_cast<int>(prev.pitch));
        if (leap <= max_leap)
          continue;
        ...
        for (int octave = 0; octave <= 10; ++octave) {
          ...
          if (candidate < static_cast<int>(range_lo) || candidate > static_cast<int>(range_hi)) {
            continue;
          }
        ...
        if (best_candidate >= 0 && static_cast<uint8_t>(best_candidate) != curr.pitch) {
  ...
  // avoid O(n^2) blowup in large pieces (passacaglia, etc.).
  {
    ...
    for (const auto& dropped : dropped_flexible) {
      if (rescue_count >= kMaxRescues)
        break;
      auto iter = structural_at_tick.find(dropped.start_tick);
      if (iter == structural_at_tick.end())
        continue;

      for (size_t si : iter->second) {
        auto& structural = result[si];
        if (structural.voice == dropped.voice)
          continue;

        int interval =
            std::abs(static_cast<int>(structural.pitch) - static_cast<int>(dropped.pitch));
        ...
        for (auto rit = result.rbegin(); rit != result.rend(); ++rit) {
          if (rit->voice == structural.voice && rit->start_tick < structural.start_tick) {
            prev_pitch = rit->pitch;
            if (structural.pitch > rit->pitch)
              prev_dir = 1;
            else if (structural.pitch < rit->pitch)
              prev_dir = -1;
            break;
          }
        }
        ...
        for (int delta : {-1, 1, -2, 2}) {
          ...
          if (prev_dir != 0 && prev_pitch > 0) {
            int new_dir = (cand > static_cast<int>(prev_pitch))   ? 1
                          : (cand < static_cast<int>(prev_pitch)) ? -1
                                                                  : 0;
            if (new_dir != 0 && new_dir != prev_dir)
              continue;
          }
          ...
          if (!interval_util::isConsonance(new_simple))
            continue;
  ...
  if (local_stats.drop_rate() > 0.10f) {
    std::fprintf(stderr, "[postValidateNotes] WARNING: drop_rate=%.1f%% (%u/%u dropped)\n",
                 local_stats.drop_rate() * 100.0f, local_stats.dropped, local_stats.total_input);
  }
...
std::vector<NoteEvent> postValidateNotes(
  ...
  return postValidateNotes(std::move(raw_notes), num_voices, key_sig, range_fn, stats,
                           protection_overrides, policy, cadence_tick);
}
...
std::vector<NoteEvent> postValidateNotes(
    ...
    std::function<std::pair<uint8_t, uint8_t>(uint8_t voice, Tick tick)> voice_range_fn,
    PostValidateStats* stats, const ProtectionOverrides& protection_overrides,
    const PostValidatePolicy& policy, Tick cadence_tick) {
  ...
  for (size_t idx = 0; idx < raw_notes.size(); ++idx) {
    int shift =
        std::abs(static_cast<int>(raw_notes[idx].pitch) - static_cast<int>(original_pitches[idx]));
    if (shift > 0) {
      repaired++;
      total_shift += shift;
      shift_count++;
      ...
      if (src == BachNoteSource::SubjectCore || src == BachNoteSource::FugueSubject ||
  ...
  local_stats.avg_shift_semitones =
      shift_count > 0 ? static_cast<float>(total_shift) / static_cast<float>(shift_count) : 0.0f;
